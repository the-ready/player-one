...
# --- 親向けの抜粋（`--for parent`）------------------------------------------
#
# 子だけでなく**親も SKILL.md 全文を毎ターン運んでいた**（3スキル中いちばん長い
# lives でも数万字ある。正確な文字数は本文の版で動くので固定しない）。全文が
...
#     **1件書けるたびに追記する（>>）。最後にまとめて書かない**——あなたは60ターンで
#     打ち切られることがあり、打ち切られるのは応答であってファイルではない。
#     逐次追記していればそこまでの成果は残り、まとめ書きだと全部失われる
