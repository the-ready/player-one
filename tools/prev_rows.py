...
という工程を持つ。しかし前回CSVをそのまま読み込むと、着手した時点でコンテキストの
数割を失う。文字数を固定の数値では書かない——`data/*.csv` は週次収集で行数が
増減し続けるため、書いた瞬間から古くなる。今の量は次で実測する（`wc -m` は
`LANG` 未設定だとバイト数を返すことがあるため使わない）：

    python3 -c "print(len(open('data/events.csv',encoding='utf-8').read()))"

append_rows.py の冒頭に書いた「後半の調査品質が落ちる」問題が、そのまま再発する。
...
def _build_carry_obj(row, lineups):
    ...
    「確認できないまま前回値を残す」という同じ操作**であり、規則を2か所に
