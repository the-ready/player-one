...
制約に対して、「節約する」以外の答えを出せる唯一の経路である。
  ...
  --text      **本文だけ**（表はタブ区切りで残る。既定で先頭 4,000 文字まで。
              切れたら残り字数が出るので、足りなければ `--limit` で増やす）
...
# `--text` / `--schedule` が標準出力に流す既定の文字数。
#
# 20,000 だったが、**実測ではこの線に一度も触れていなかった**（2026-09-04 の
# 取得68回：中央値2,121字・平均2,345字・最大7,191字）。つまり上限が効いていた
# ページは0件で、事実上「無制限」だった。取得結果は子の文脈に積み上がり、以後の
# 全ターンで再送されるので、**どこかに歯止めが要る。**
#
# 4,000 にしてある。実測の分布に当てると、切られるのは68件中10件（全体の9%）で、
# 中央値のページには一切触れない。**2,000 まで下げれば35%減るが、そこは中央値の
# ページを切りにいく線**であり、一覧の後半に載っている公演や料金の行を落とす
# ——トークンのために調査品質を落とすのは、この道具でいちばんやってはいけない誤りである。
# 切ったときは残り字数と `--limit` の案内が出るので、足りなければその場で増やせる。
DEFAULT_LIMIT = 4000             # --text / --schedule の既定の出力文字数
