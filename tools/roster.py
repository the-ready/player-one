...
## なぜスキル本文の目標件数・品質基準・禁止事項を書き換えないのか

会場は開業し、閉館し、改称する。名画座は畳み、フェスは開催地を変える。
定点観測リストは放っておけば必ず腐るので、収集タスク自身が更新できるほうがよい。

ただし **SKILL.md の目標件数・品質基準・禁止事項を実行のたびに書き換える形は採らない。**
理由は4つある。
...
それらに触れない小さなバグ（誤字脱字・壊れたコマンド例・古いパス参照等）は、
収集タスク自身が SKILL.md・`tools/`・対応する設計文書を直接修正してよい
（`docs/skill-feedback.md` の「自己修正ログ」に記録する。
`docs/COLLECTION-PROTOCOL.md` 第7.1節）。
...
def record_hits(kind, names, today=None):
    ...
    そのうえ `ACTIVE_TTL=180` 日が来れば、収穫の有無にかかわらず全件が
