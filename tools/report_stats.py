...
#
# 過去5回の実測（18% / 96% / 41% / 15% / 32%）のうち唯一まともだった回が96%で、
# 他は明確に調査が届いていない回だった。当初はそこから70%に置いたが、96%との
# 差を詰めるため85%に引き上げている。
# **「毎週落ちるなら下限が高すぎる」ではなく「毎週調べられていない」が正しい読み方**
# であることを、この数字で固定する。
#
# **events にしか下限が無かったせいで、この門は3スキル中2つで無効だった。**
# 2026-09-04 20:50 の lives 収集は、新規77行の `onsale_start`/`onsale_end` が0%・
# `onsale_label` が5%でも `--check-fresh` が exit 0 で通り、そのままコミットされた。
# lives / movies にも下限を置く。値は実測から採る（git履歴の各回について、
# 「前回に無い uid の行」だけで数えたもの）。
#
#   lives  onsale_label : 良い回 80% / 100% / 100%、崩れた回 0% / 5%
#          desc         : 良い回 100%×3、崩れた回 19%
#   movies price        : 実測 100%
#          desc         : 実測 100%
#
# 下限はどれも実測の「良い回」よりかなり低い側に置いてある。**この門はコミットを
# 止めない**（`verify-data.sh` が1回だけ強く催促するだけで、`claude-routine.sh` の
# コミット判定には入れていない）ので、外したときの費用はターン1回で済む。
