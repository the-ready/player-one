...
import { txt, num, int, bool, fold, stableUid } from "./util.js";
...
/* ---------- 日割りラインナップ（data/lineups.csv・第12.12節） ----------

   フェスは1公演＝1行なので、lives.csv の `artists` 列には主要な出演者しか入らない
   （200組を1セルに詰めるとカードもCSVも読めなくなる）。そこで「どの日に誰が出るか」
   だけを別ファイルに逃がし、`lives.csv` の `lineup_id` で引く。

   本体データと同時に読む（遅延読み込みにしない）。理由は2つある。
     ・出演者数をカード上に出したい。開くまで分からないと押す価値が伝わらない
     ・全出演者を検索の索引に入れたい（searchHay）。「Vaundy」で検索した人に、
       その名前がカードに出ていないフェスを見つけさせるのがこの層の主目的である
   読めなくても本体は動く（ボタンが出ないだけ）。 */

const LINEUPS = new Map(); // lineup_id -> 行の配列（CSVの並び順を保つ）
...
export async function loadLineups() {
  ...
  lineupsLoaded = true;
  try {
    const { text } = await fetchText("./data/lineups.csv");
    parseCsvObjects(text).forEach((row) => {
      const id = txt(row.lineup_id);
      const artist = txt(row.artist);
      if (!id || !artist) return;
      if (!LINEUPS.has(id)) LINEUPS.set(id, []);
      LINEUPS.get(id).push({
        date: txt(row.date),
        stage: txt(row.stage),
        artist,
        isHeadliner: bool(row.is_headliner),
        appleMusicUrl: txt(row.apple_music_url),
        note: txt(row.note),
      });
    });
  } catch {
    /* 「全出演者を見る」が出ないだけで、カードもタブも従来どおり表示される */
  }
}

/** その公演の日割り行（CSVの並び＝出演順・格上順を保つ）。 */
export function lineupRows(id) {
  return (id && LINEUPS.get(id)) || [];
}

/* 重複を除いた出演者名。同じアーティストが複数日に出ることがある。
   検索の索引づくりとカードの描画の両方から引かれ、1件が最大200組になるので、
   打鍵のたびに畳み直さないよう lineup_id 単位で覚えておく（CSVは読み込み後に
   変わらないので、無効化の仕組みは要らない）。 */
const artistNameCache = new Map();
export function lineupArtistNames(id) {
  if (!id) return [];
  if (!artistNameCache.has(id))
    artistNameCache.set(id, [...new Set(lineupRows(id).map((r) => r.artist))]);
  return artistNameCache.get(id);
}

export const lineupCount = (id) => lineupArtistNames(id).length;
...
function searchHay(tab, it) {
  ...
  return fold(
    [it.title, it.kana, it.venue, it.area, it.desc, it.note]
      .concat(it.venues || [])
      .concat(it.artists || [])
      // フェスは artists 列が主要8組までなので、それだけを索引にすると
      // 「出演するのに検索で出てこないアーティスト」が大量に生まれる。
      // 日割りラインナップ（第12.12節）の全出演者を索引に含める。
      .concat(lineupArtistNames(it.lineupId))
      .concat(labels)
      .filter(Boolean)
      .join(" "),
  );
...
/* schedule.js の refreshToday() をラップし、日付が変わったときに検索インデックス
   ...
   「本日開催」に変わっているのに検索窓に「本日開催」と打っても見つからない、
...
export async function loadTab(tabKey) {
  ...
  try {
    // ラインナップは searchHay() が使うので、行を写す前に読み終えておく。
    // あとから読むと、検索の索引だけが出演者を知らないままになる。
    if (tabKey === "live") await loadLineups();
