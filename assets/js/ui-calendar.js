...
function baseToday(){ return new Date(TODAY() + "T00:00:00"); }
...
export function presetRange(key){
  ...
  if(key === "weekend"){
    const dow = t.getDay();
    const sat = new Date(t); sat.setDate(t.getDate() + ((6 - dow) + 7) % 7);
    const sun = new Date(sat); sun.setDate(sat.getDate() + 1);
    return [iso(dow === 0 ? t : sat), iso(dow === 0 ? t : sun)];
  }
  if(key === "7d"){ const e = new Date(t); e.setDate(t.getDate() + 6); return [iso(t), iso(e)]; }
  if(key === "month"){ const e = new Date(t.getFullYear(), t.getMonth() + 1, 0); return [iso(t), iso(e)]; }
  const e = new Date(t); e.setMonth(t.getMonth() + 3); return [iso(t), iso(e)];
}

function ensureCursor(){
  const st = curState();
  const anchor = st.rangeStart || TODAY();
  if(!calCursor) calCursor = new Date(anchor.slice(0, 4), parseInt(anchor.slice(5, 7), 10) - 1, 1);
...
export function initCalendar(){
  ...
  document.getElementById("calClear").addEventListener("click", () => {
    const st = curState();
    st.rangeStart = null; st.rangeEnd = null; pendingStart = null;
    renderCalendar(); refreshNow({push:true});
  });

  const presetWrap = document.getElementById("calPresets");
  presetWrap.innerHTML = PRESETS.map(p =>
    `<button type="button" class="preset-chip" data-key="${p.key}" aria-pressed="false">${p.label}</button>`).join("");
  presetWrap.addEventListener("click", e => {
    const btn = e.target.closest(".preset-chip");
    if(!btn) return;
    const [s, en] = presetRange(btn.dataset.key);
    const st = curState();
    st.rangeStart = s; st.rangeEnd = en; pendingStart = null;
  ...
  document.getElementById("sortDateChip").addEventListener("click", () => setSort("date"));
  document.getElementById("sortAnnouncedChip").addEventListener("click", () => setSort("announced"));
  ...
  grid.addEventListener("focusin", () => { gridHadFocus = true; });
  grid.addEventListener("focusout", () => { gridHadFocus = false; });
}
...
function pickDate(dateStr){
  ...
  focusDate = dateStr;
  if(!pendingStart){
    pendingStart = dateStr; st.rangeStart = null; st.rangeEnd = null;
    document.getElementById("calSelectionText").textContent = `${fmtDateDots(dateStr)} 〜 終了日を選んでください`;
    renderCalendar();
...
function onGridKey(e){
  ...
  switch(e.key){
    ...
    case "Home":       next = new Date(cur); next.setDate(cur.getDate() - cur.getDay()); break;
    case "End":        next = new Date(cur); next.setDate(cur.getDate() + (6 - cur.getDay())); break;
    case "PageUp":     next = new Date(cur); next.setMonth(cur.getMonth() - 1); break;
    case "PageDown":   next = new Date(cur); next.setMonth(cur.getMonth() + 1); break;
    default: return;
  }
  e.preventDefault();
  focusDate = iso(next);
...
export function renderCalendar(restoreFocus){
  ...
  const s = st.rangeStart, e = st.rangeEnd;
  const dayCls = date => {
  ...
  let html = `<div class="cal-row" role="row">` +
    WD.map(d => `<span class="cal-dow" role="columnheader" aria-label="${d}曜日">${d}</span>`).join("") +
    `</div>`;
  for(let i = 0; i < cells.length; i += 7){
    html += `<div class="cal-row" role="row">` + cells.slice(i, i + 7).map(date => {
  ...
  const hadFocus = restoreFocus || gridHadFocus || grid.contains(document.activeElement);
  grid.innerHTML = html;
  ...
  if(!pendingStart){
    sel.textContent = (st.rangeStart && st.rangeEnd)
  ...
  document.querySelectorAll("#calPresets .preset-chip").forEach(c => {
    ...
    c.setAttribute("aria-pressed", String(st.rangeStart === ps && st.rangeEnd === pe));
  });
  document.getElementById("sortDateChip").setAttribute("aria-pressed", String(st.sortBy !== "announced"));
  document.getElementById("sortAnnouncedChip").setAttribute("aria-pressed", String(st.sortBy === "announced"));
}
...
export function resetCalendarCursor(){
  ...
  calCursor = new Date(anchor.slice(0, 4), parseInt(anchor.slice(5, 7), 10) - 1, 1);
  focusDate = anchor;
}
