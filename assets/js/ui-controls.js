...
import { syncPrefChips, renderVenueList, updateMapAreaStatus, syncLocateMsg } from "./ui-area.js";
...
export function initControls(){
  ...
  el.q.addEventListener("input", e => { curState().q = e.target.value; refresh({push:false}); });
  el.q.addEventListener("keydown", e => {
    if(e.key === "Escape" && el.controls.classList.contains("search-open")){
      e.preventDefault(); closeSearch();
    }
  });
  el.q.addEventListener("blur", () => { if(!el.q.value) closeSearch(); });
...
export function resetFilters(){
  ...
  resetCalendarCursor();
  renderCalendar();
  syncPrefChips(); renderVenueList(); updateMapAreaStatus(); syncLocateMsg();
...
function removeFilter(type, facetId, key){
  const st = curState();
  switch(type){
    case "q":     st.q = ""; el.q.value = ""; break;
    case "range": st.rangeStart = null; st.rangeEnd = null; renderCalendar(); break;
    case "sort":  st.sortBy = "date"; if(st.userLoc) st.userLoc = null; syncLocateMsg(); renderCalendar(); break;
    case "pref":  st.pref = null; syncPrefChips(); break;
    case "venue": st.venue = null; renderVenueList(); break;
    case "map":   st.mapArea = null; updateMapAreaStatus(); break;
    case "facet": st.sets[facetId]?.delete(key); break;
    case "flag":  st.flags[key] = false; break;
  }
  refreshNow({push:true});
}
...
export function updateSortUI(){
  ...
  el.sortDate.setAttribute("aria-pressed", String(hasRange || st.sortBy === "announced"));
  el.sortLoc.setAttribute("aria-pressed", String(!!(st.pref || st.venue || st.mapArea || st.sortBy === "location")));
  el.cat.setAttribute("aria-pressed", String(
    tab.facets.some(f => st.sets[f.id].size) || tab.flags.some(f => st.flags[f.id])));
