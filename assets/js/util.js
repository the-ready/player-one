...
export const txt  = v => { const s = (v == null ? "" : String(v)).trim(); return s ? s : null; };
...
export const list = v => { const s = txt(v); return s ? s.split("|").map(x => x.trim()).filter(Boolean) : []; };
// "0" や "false" を真と誤認しないよう、明示的に真を表す値だけを拾う。
export const bool = v => ["1","true","yes"].includes(String(v == null ? "" : v).trim().toLowerCase());
...
export function fmtKm(km){ return km < 10 ? `${km.toFixed(1)}km` : `${Math.round(km)}km`; }
...
export function foldWithMap(src){
  ...
  for(let i = 0; i < s.length; i++){
    let unit = s[i], consumed = 1;
    // 半角カナ＋濁点/半濁点は2文字で1文字ぶん。先に合成しないと "ﾃﾞ" が "て゛" になる
    if(HALFWIDTH_KANA.test(unit) && i+1 < s.length && VOICED_MARK.test(s[i+1])){
      unit = s[i] + s[i+1];
      consumed = 2;
    }
    const folded = foldUnit(unit);
    for(const ch of folded){ text += ch; map.push(i); }
    if(consumed === 2) i++;
  }
...
export function fold(src){ return foldWithMap(src).text; }

/** 検索語をAND条件の配列に割る。「横浜 ロック」→ ["よこはま","ろっく"] */
export function searchTerms(q){
  return fold(q).split(/[\s　]+/).map(t => t.trim()).filter(Boolean);
}
...
export function highlight(src, terms){
  ...
  for(const term of terms){
    ...
    for(;;){
      ...
      if(at < 0) break;
      for(let k = at; k < at + term.length && k < map.length; k++) hits[map[k]] = true;
      from = at + term.length;
    }
  }
  let out = "", open = false;
  for(let i = 0; i < s.length; i++){
    ...
    else if(!hits[i] && open){ out += "</mark>"; open = false; }
    out += esc(s[i]);
  }
