...
import { esc, safeUrl, fmtDateDots, fmtKm, searchTerms, debounce } from "./util.js";
import { TABS, TAB_ORDER, PREFS, venueNames } from "./config.js";
import { ITEMS, LOAD, SOURCES, updatedFor, dataStatus } from "./data.js";
import { matchesFilters, withDistances, sortItems, facetCounts, flagCounts } from "./filters.js";
import { cardHtml, buildIcs, gcalUrl, ICON as CAL_ICON } from "./cards.js";
import { STATES, activeTab, curTab, curState, toggleFav, hasAnyFilter, syncUrl } from "./state.js";
...
export function visibleItems(tabKey = activeTab){
  const tab = TABS[tabKey], st = STATES[tabKey];
...
function itemFromEl(el){
  ...
  return lastVisible.find(it => it.key === key) || ITEMS[activeTab].find(it => it.key === key) || null;
}

/* ---------- カード内の操作（委譲で1度だけ束ねる） ---------- */

export function bindList(listEl, {onOpenPlace, onReset}){
  listEl.addEventListener("click", e => {
    ...
    if(toggle){
      ...
      toggle.setAttribute("aria-label", open ? `${toggle.dataset.title}の説明をとじる` : `${toggle.dataset.title}のくわしい説明を見る`);
      toggle.textContent = open ? "とじる ▴" : "くわしく ▾";
      return;
    }
    const place = e.target.closest("button.place-link[data-place]");
    if(place){ onOpenPlace(place.dataset.place); return; }

    const act = e.target.closest(".act-btn, .empty-reset");
    if(!act) return;
    const kind = act.dataset.act;
    if(kind === "reset"){ onReset(); return; }
  ...
  // ポスターURLが実在しない/ホットリンクを拒否された場合に、壊れた画像アイコンではなく
  // チケット風のプレースホルダーへ切り替える。error はバブルしないので捕捉フェーズで拾う。
  listEl.addEventListener("error", e => {
    const img = e.target;
    if(!(img instanceof HTMLImageElement) || !img.hasAttribute("data-poster")) return;
    img.closest(".ticket-poster")?.classList.add("no-poster");
    img.remove();
  }, true);
}
...
function doFav(item, btn){
  ...
  btn.setAttribute("aria-label", `${item.title}を行きたいリストに${now ? "登録済み。外す" : "追加する"}`);
  btn.setAttribute("title", now ? "行きたいリストから外す" : "行きたいリストに追加");
...
async function doShare(item){
  ...
  const text = [item.date, venueNames(item).join("・") || item.area].filter(Boolean).join(" / ");
...
function openCalMenu(btn, item){
  if(calMenuEl && calMenuEl.owner === btn){ closeCalMenu(); btn.focus(); return; }
  closeCalMenu();
  ...
  let top = r.bottom + 6;
  if(top + m.height > window.innerHeight - pad) top = Math.max(pad, r.top - m.height - 6);
  ...
  menu.addEventListener("keydown", e => {
    if(e.key === "Escape"){ e.preventDefault(); closeCalMenu(); btn.focus(); return; }
...
document.addEventListener("pointerdown", e => {
  if(!calMenuEl) return;
  if(calMenuEl.contains(e.target) || calMenuEl.owner.contains(e.target)) return;
  closeCalMenu();
}, true);
...
window.addEventListener("scroll", () => {
  if(calMenuEl && Math.abs(window.scrollY - calMenuScrollY) > 24) closeCalMenu();
}, {passive:true});
window.addEventListener("resize", () => closeCalMenu());

function doIcs(item){
  const ics = buildIcs(item);
  if(!ics){ toast("日付が未登録のためカレンダーに追加できません"); return; }
...
export function renderFilterChips(){
  ...
  tab.flags.forEach(f => {
    ...
    if(!g){ g = {name:f.group, flags:[]}; groups.push(g); }
    g.flags.push(f);
  });

  const flagRows = groups.map(g => `
    <div class="chip-row">
      <span class="chip-label">${esc(g.name)}</span>
      ${g.flags.map(f => chipButton({
        key:f.id, text:f.label(tab), count:gCounts[f.id], pressed:st.flags[f.id], cls:`flag-chip ${f.cls}`
      })).join("")}
    </div>`).join("");

  const facetRows = tab.facets.map(f => {
    ...
    return `<div class="chip-row" data-facet="${esc(f.id)}">
      ...
      ${keys.map(k => {
        const meta = f.meta(k);
        return chipButton({
          key:k, text:meta.text, count:fCounts[f.id][k] || 0,
          pressed:st.sets[f.id].has(k), color:meta.c, textColor:meta.tx, cls:"facet-chip"
        });
      }).join("")}
    </div>`;
  }).join("");
  ...
  const focusKey = document.activeElement && body.contains(document.activeElement)
...
export function appliedFilters(){
  ...
  if(st.rangeStart && st.rangeEnd)
    out.push({type:"range", label:`${fmtDateDots(st.rangeStart)}〜${fmtDateDots(st.rangeEnd)}`});
  if(st.sortBy === "announced") out.push({type:"sort", label:"新着順"});
  if(st.sortBy === "location") out.push({type:"sort", label:"現在地から近い順"});
  if(st.pref) out.push({type:"pref", label:(PREFS.find(p => p.key === st.pref) || {}).label || st.pref});
  if(st.venue) out.push({type:"venue", label:st.venue});
  if(st.mapArea) out.push({type:"map", label:`地図 半径${fmtKm(st.mapArea.radiusKm)}`});
  tab.facets.forEach(f => st.sets[f.id].forEach(k =>
    out.push({type:"facet", facet:f.id, key:k, label:f.meta(k).text})));
  tab.flags.forEach(f => { if(st.flags[f.id]) out.push({type:"flag", key:f.id, label:f.label(tab)}); });
  return out;
}
...
export function renderAppliedBar(){
  ...
  if(!applied.length){ bar.innerHTML = ""; return; }
  bar.innerHTML =
    `<span class="applied-label">絞り込み中</span>` +
    applied.map(a => `<button type="button" class="applied-chip" data-type="${esc(a.type)}"
        data-facet="${esc(a.facet || "")}" data-key="${esc(a.key || "")}"
        aria-label="${esc(a.label)}の絞り込みを外す">${esc(a.label)}<span aria-hidden="true">×</span></button>`).join("") +
    `<button type="button" class="applied-clear" id="appliedClear">すべて解除</button>`;
}

/* ---------- ヘッダーの最終更新日 ---------- */
const TAB_DATA_NOUNS = {event:"イベント", movie:"映画", live:"ライブ・フェス"};
export function syncUpdatedLabel(){
  ...
  document.getElementById("statUpdatedChip")
    ?.setAttribute("aria-label", `${TAB_DATA_NOUNS[activeTab] || ""}データの最終更新日 ${shown}`);
...
const uiHooks = [];
export function onRefresh(fn){ uiHooks.push(fn); }
