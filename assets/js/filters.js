...
/* 受付の開始・終了は「日付」ではなく「時刻まで含めた瞬間」で判定する。
   ...
   1. **時・分を2桁に揃える。** CSVの時刻列の書式は `H:MM`（設計書 第3.9.2節）で、
      実データにも `9:00` `8:30` が入っている。ところが `new Date()` に渡す
      ISO8601 は時が2桁でないと **Invalid Date** になる。null に落ちると
      「開始時刻が読めない＝発売前ではない」と解釈され、まだ発売していない公演が
      受付中として表示され、「受付中のものだけ」の絞り込みにも通ってしまう
      ——この関数がまさに防ごうとしている嘘を、書式のずれで再生産することになる。
   2. **日本時間として読む。** CSVの日時はすべて日本時間で書かれている。
      タイムゾーンを省くと閲覧者の端末の時刻として解釈されるので、日本国外から
      見た人には締切が数時間ずれる（第3.9.3節が「本日」を日本時間に固定したのと
      同じ理由。基準はデータ側の暦にそろえる）。 */
...
function onsaleAt(ymd, hm, fallbackHM) {
  if (!ymd) return null;
  const t = new Date(`${ymd}T${hhmm(hm) || fallbackHM}:00${JST_OFFSET}`);
  return isNaN(t.getTime()) ? null : t;
}
export const onsaleStartAt = (it) =>
  onsaleAt(it.onsaleStart, it.onsaleStartTime, "00:00");
export const onsaleEndAt = (it) =>
  onsaleAt(it.onsaleEnd, it.onsaleEndTime, "23:59");
...
/* 終了日を過ぎている行。開催状況を「本日」から計算するようにしたことで、
   ...
   終わった催しは既定では一覧から外す——探しているのは「これから行けるもの」が
   大半で、終了分が混じるほど本題が埋もれるため。ただし完全には消さない。
   「載っていない」と「終わった」を区別できなくなるのは別の害なので、
   `showEnded` フラグ（絞り込みの「終了のものも表示」チップ）で戻せるようにしてある
   （下の `matchesEndedFlag`）。

   予備日がまだ残っている行は、会期を過ぎていても終わったことにしない
   （本開催が流れていればその日にやる。schedule.js の backup 段階と同じ理由）。 */
export const hasEnded = (it) =>
  !!(
    it.endDate &&
    it.endDate < TODAY() &&
...
const FLAG_HELPERS = { onsaleState, isNewlyAnnounced, isFav, hasEnded };
...
function matchesFlags(tab, st, it, exceptId) {
  return tab.flags.every((f) => {
    // showEnded は他のフラグと on/off の意味が逆（立てると絞り込みが緩む）ので、
    // この汎用ループでは扱わず matchesEndedFlag に任せる。
    if (f.id === exceptId || f.id === "showEnded") return true;
    if (!st.flags[f.id]) return true;
    return f.test(it, FLAG_HELPERS);
  });
}

// 終了した行の既定非表示。matchesFlags の他のフラグは「立てたら絞り込む」向きだが、
// これは「立てたら緩む」逆向きなので専用の関数にしてある。
function matchesEndedFlag(st, it) {
  return st.flags.showEnded || !hasEnded(it);
}
...
export function matchesBaseFilters(tab, st, it) {
  return (
    matchesQuery(st, it) &&
    matchesDate(st, it) &&
    matchesFacets(tab, st, it) &&
    matchesFlags(tab, st, it) &&
    matchesEndedFlag(st, it)
  );
}
...
export function sortItems(items, st) {
  if (st.sortBy === "location" && st.userLoc)
    return items.slice().sort((a, b) => a._dist - b._dist);
...
export function flagCounts(tab, st, items) {
  const out = {};
  tab.flags.forEach((f) => {
    // showEnded は「立てると何件増えるか」を数えたいので、既定非表示の判定
    // （matchesEndedFlag）自体は外し、代わりに終了行だけを対象にする。
    if (f.id === "showEnded") {
      out[f.id] = items.filter(
        (it) =>
          matchesQuery(st, it) &&
          matchesDate(st, it) &&
          matchesFacets(tab, st, it) &&
    ...
    out[f.id] = items.filter(
      (it) =>
        ...
        matchesDate(st, it) &&
        matchesFacets(tab, st, it) &&
        matchesFlags(tab, st, it, f.id) &&
        matchesEndedFlag(st, it) &&
        matchesArea(st, it) &&
        f.test(it, FLAG_HELPERS),
    ).length;
  });
  return out;
}
