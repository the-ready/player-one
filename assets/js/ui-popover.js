...
export function setBackgroundInert(on, extra = []){
  INERT_TARGETS().concat(extra).forEach(el => {
    if(!el) return;
...
export function initPopover({onOpen, onClose}){
  ...
  window.addEventListener("resize", () => { if(currentPopover) positionPopover(currentPopover); });
...
function positionPopover(which){
  if(!desktopMQ()){ el.panel.style.left = ""; return; }
  ...
  const left = Math.min(Math.max(0, btnRect.left - containerRect.left), maxLeft);
  el.panel.style.left = `${left}px`;
}

export function isPopoverOpen(){ return !el.panel.hidden; }
export function currentPopoverName(){ return currentPopover; }
...
export function openPopover(which){
  ...
  Object.entries(el.bodies).forEach(([k, body]) => { body.hidden = k !== which; });
  Object.entries(el.btns).forEach(([k, btn]) => btn.setAttribute("aria-expanded", String(k === which)));
  Object.entries(el.arrows).forEach(([k, a]) => { a.textContent = k === which ? "▴" : "▾"; });
...
export function closePopover(){
  if(el.panel.hidden){ currentPopover = null; return; }
  const back = (returnFocus && returnFocus.offsetParent !== null) ? returnFocus : el.btns[currentPopover];
  currentPopover = null;
  el.backdrop.hidden = true;
  el.panel.hidden = true;
  el.panel.classList.remove("show");
  Object.values(el.btns).forEach(b => b.setAttribute("aria-expanded", "false"));
  Object.values(el.arrows).forEach(a => { a.textContent = "▾"; });
...
export function togglePopover(which){
  if(!el.panel.hidden && el.btns[which].getAttribute("aria-expanded") === "true") closePopover();
  else {
...
export function handleScrollForPopover(){
  ...
  if(el.panel.contains(document.activeElement)){ openScrollY = window.scrollY; return; }
