...
const LIVE_COLUMNS = {
  ...
  parking: ["parking", txt],
  nearestStation: ["nearest_station", txt],
  // メインアーティストの Apple Music アーティストページ（第12.11節）。
  appleMusicUrl: ["apple_music_url", txt],
  // data/lineups.csv の同じ値の行が、この公演の日割りラインナップになる（第12.12節）。
  lineupId: ["lineup_id", txt],
};
...
/* 終了した行は既定で一覧から隠す（filters.js の matchesEndedFlag）。
   「載っていない」と「終わった」の区別が付かなくなるのを避けるため、
   完全に消すのではなくこのチップで戻せるようにしてある。
   test は「このチップを立てたときに何件増えるか」の集計（flagCounts）専用で、
   実際の絞り込みは matchesEndedFlag が別枠で行う（他のチップとは on/off の意味が逆のため）。 */
const FLAG_SHOW_ENDED = {
  id: "showEnded",
  group: "表示",
  cls: "ended-chip",
  label: (t) => `${STATUS_BY_PHASE[t.key]?.ended || "終了"}のものも表示`,
  test: (it, h) => h.hasEnded(it),
};
...
export const TABS = {
  event: {
    ...
    // カード左上のバッジと本文ピルに使う「カテゴリ」列（先頭がバッジ、残りがピル）
    cat: { field: "cats", table: CATS },
    // カテゴリとは別軸の「形態」ピル。イベントは持たない
    type: null,
    cta: {
      suffix: "で予約する",
      fallback: "予約ページを開く",
  ...
  movie: {
    ...
    cat: { field: "genre", table: MOVIE_GENRES },
    type: { field: "screeningType", table: SCREENING_TYPES },
    cta: {
      suffix: "で上映情報を見る",
      fallback: "上映情報を見る",
      ariaWith: "で上映情報を見る",
  ...
  live: {
    ...
    cat: { field: "genre", table: LIVE_GENRES },
    type: { field: "liveType", table: LIVE_TYPES },
    // 単日公演が多く日付そのものが主情報なので、日付だけ一段大きく出す
    dateClass: "live-date",
    cta: {
      suffix: "でチケットを見る",
      fallback: "チケットページを開く",
