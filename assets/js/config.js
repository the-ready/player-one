...
const presentKeys = (tbl, getter) => items => {
  ...
  return Object.keys(tbl).filter(k => present.has(k))
...
const FLAG_NEW     = {id:"newOnly",     group:"注目",   cls:"add-chip",
                      label:t => `新着・${t.additionalLabel}だけ`,
                      test:(it,h) => it.isAdditional || h.isNewlyAnnounced(it)};
const FLAG_ONSALE  = {id:"onsaleOnly",  group:"注目",   cls:"onsale-chip", exclusive:["beforeOnly"],
                      label:t => `${t.onsaleWords.open}のものだけ`,
                      test:(it,h) => h.onsaleState(it) === "open"};
const FLAG_BEFORE  = {id:"beforeOnly",  group:"注目",   cls:"before-chip", exclusive:["onsaleOnly"],
                      label:t => `${t.onsaleWords.before}のものだけ`,
                      test:(it,h) => h.onsaleState(it) === "before"};
const FLAG_LIMITED = {id:"limitedOnly", group:"注目",   cls:"limited-chip",
                      label:() => "限定・追加販売あり",
                      test:it => !!it.limitedSale};
const FLAG_DEALS   = {id:"dealsOnly",   group:"おトク", cls:"deal-chip",
                      label:() => "割引・クーポンがあるものだけ",
                      test:it => !!(it.discountPct || it.couponNote)};
const FLAG_FAV     = {id:"favOnly",     group:"保存",   cls:"fav-chip",
                      label:() => "お気に入りだけ",
...
export const TABS = {
  event: {
    ...
    keyPrefix:"e",
    paneId:"eventPane", listId:"list", btnId:"tabEventBtn",
    dirGridId:"dirGrid", dirCountId:"dirCount",
    placeholder:"イベント名・会場・エリアで検索",
    catTitle:"イベントをしぼる",
    emptyTitle:"該当するイベントが見つかりません",
    statusTable:EVENT_STATUS_STYLE,
    cardClass:"", bodyClass:"card-body",
    // カード上部の意匠（イベントは絵柄を持たない）
    header:null,
    cta:{suffix:"で予約する", fallback:"予約ページを開く", ariaWith:"で予約する", ariaWithout:"予約ページを開く"},
    ...
    facets:[
      facet("cats", "カテゴリ", it => it.cats, pillMeta(CATS), () => Object.keys(CATS)),
      facet("statuses", "開催状況", statusOf, statusChipMeta(EVENT_STATUS_STYLE), presentKeys(EVENT_STATUS_STYLE, statusOf))
    ],
    flags:[FLAG_DEALS, FLAG_NEW, FLAG_ONSALE, FLAG_BEFORE, FLAG_LIMITED, FLAG_FAV]
  },
  ...
  movie: {
    ...
    keyPrefix:"m",
    paneId:"moviePane", listId:"movieList", btnId:"tabMovieBtn",
    dirGridId:"movieDirGrid", dirCountId:"movieDirCount",
    placeholder:"映画名・劇場名・エリアで検索",
    catTitle:"映画をしぼる",
    emptyTitle:"該当する映画が見つかりません",
    statusTable:MOVIE_STATUS_STYLE,
    cardClass:"ticket-card", bodyClass:"card-body ticket-body",
    header:"poster",
    cta:{suffix:"で上映情報を見る", fallback:"上映情報を見る", ariaWith:"で上映情報を見る", ariaWithout:"上映情報を見る"},
    officialLabel:"作品公式サイトを見る",
    additionalLabel:"上映延長",
    seriesLabel:"この特集の他の上映",
    onsaleWords:{before:"前売り発売前", open:"前売り発売中", closed:"前売り終了"},
    onsaleNoun:"前売り",
    hasPrice:true,
    place:{
      pickerLabel:"劇場でしぼる",
    ...
    facets:[
      facet("genres", "ジャンル", it => it.genre, pillMeta(MOVIE_GENRES), () => Object.keys(MOVIE_GENRES)),
      facet("screeningTypes", "上映形態", it => it.screeningType, pillMeta(SCREENING_TYPES), () => Object.keys(SCREENING_TYPES)),
      facet("statuses", "上映状況", statusOf, statusChipMeta(MOVIE_STATUS_STYLE), presentKeys(MOVIE_STATUS_STYLE, statusOf))
    ],
    flags:[FLAG_DEALS, FLAG_NEW, FLAG_ONSALE, FLAG_BEFORE, FLAG_LIMITED, FLAG_FAV]
  },
  ...
  live: {
    ...
    keyPrefix:"l",
    paneId:"livePane", listId:"liveList", btnId:"tabLiveBtn",
    dirGridId:"liveDirGrid", dirCountId:"liveDirCount",
    placeholder:"アーティスト名・公演名・会場で検索",
    catTitle:"公演をしぼる",
    emptyTitle:"該当する公演が見つかりません",
    statusTable:LIVE_STATUS_STYLE,
    cardClass:"live-card", bodyClass:"card-body live-body",
    header:"lineup",
    cta:{suffix:"でチケットを見る", fallback:"チケットページを開く", ariaWith:"でチケットを見る", ariaWithout:"チケットページを開く"},
    ...
    facets:[
      facet("genres", "ジャンル", it => it.genre, pillMeta(LIVE_GENRES), () => Object.keys(LIVE_GENRES)),
      facet("liveTypes", "公演形態", it => it.liveType, pillMeta(LIVE_TYPES), () => Object.keys(LIVE_TYPES)),
      facet("statuses", "開催状況", statusOf, statusChipMeta(LIVE_STATUS_STYLE), presentKeys(LIVE_STATUS_STYLE, statusOf))
    ],
...
export const GENRE_TABLE_BY_TAB = {event:CATS, movie:MOVIE_GENRES, live:LIVE_GENRES};

/** そのアイテムが持つ会場名の配列（映画は複数館ありうる）。 */
export function venueNames(it){
  if(it.venues && it.venues.length) return it.venues.filter(v => v && v !== "-");
