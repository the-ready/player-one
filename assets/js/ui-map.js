...
import { DEFAULT_CENTER, PLACE_UPCOMING_MAX, TABS, TAB_ORDER, venueNames, VENUE_KINDS } from "./config.js";
import { ITEMS, venueMeta, theaterMeta, isKnownChain, pointsForPlace } from "./data.js";
...
import { geoPrecheckError, geoErrorMessage, getPositionWithRetry, updateMapAreaStatus, renderVenueList } from "./ui-area.js";

const LEAFLET_CSS = "./assets/vendor/leaflet/leaflet.css";
const LEAFLET_JS  = "./assets/vendor/leaflet/leaflet.js";
const MAP_MIN_ZOOM = 5, MAP_MAX_ZOOM = 18;
...
function ensureLeaflet(){
  ...
  leafletPromise = new Promise((resolve, reject) => {
    ...
    s.onload  = () => window.L ? resolve() : reject(new Error("地図ライブラリを初期化できませんでした"));
    s.onerror = () => { leafletPromise = null; reject(new Error("地図ライブラリを読み込めませんでした")); };
...
const TILE_ATTR = '&copy; <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener noreferrer">OpenStreetMap</a> contributors';
...
export function initMapSheet(){
  ...
  el.sheet.addEventListener("click", e => { if(e.target === el.sheet) closeMapSheet(); });
  el.sheet.addEventListener("keydown", e => {
    if(e.key === "Escape"){ e.preventDefault(); closeMapSheet(); return; }
    trapTab(el.sheet, e);
  });

  el.apply.addEventListener("click", () => {
    if(!liveArea) return;
    setPlaceFilter(curState(), "map", {lat:liveArea.lat, lng:liveArea.lng, radiusKm:liveArea.radiusKm});
    closeMapSheet();
  ...
  el.clear.addEventListener("click", () => {
    ...
    closeMapSheet();
    updateMapAreaStatus(); refreshNow({push:true});
  });
  el.locate.addEventListener("click", onMapLocate);

  window.addEventListener("resize", () => {
    if(el.sheet.hidden || !map) return;
    map.invalidateSize(); layoutRing(); settleMap();
  });
}
...
function layoutRing(){ el.ring.style.setProperty("--ring-size", (ringRadiusPx() * 2) + "px"); }

// 円の縁までの実距離。東西方向で測ると、その緯度での縮尺をそのまま使えて誤差が出ない。
function currentRadiusKm(){
  const size = map.getSize();
  const c = map.getCenter();
  const edge = map.containerPointToLatLng([size.x / 2 + ringRadiusPx(), size.y / 2]);
...
function setMapNote(text){ el.note.textContent = text || ""; el.note.hidden = !text; }
const itemNoun = () => curTab().noun;

function defaultMapNote(){
  const n = ITEMS[activeTab].filter(it => it.lat == null || it.lng == null).length;
  return n ? `位置情報が未登録の${itemNoun()}（${n}件）は、地図でしぼりこむと表示されません。` : "";
}
function countInLiveArea(){
  const tab = curTab(), st = curState();
  return ITEMS[activeTab].filter(it => matchesBaseFilters(tab, st, it) && withinArea(it, liveArea)).length;
}
...
function updateReadout(announce){
  ...
  if(announce) el.sr.textContent = `中心から半径 約${fmtKm(liveArea.radiusKm)}。この範囲に ${n} 件の${itemNoun()}があります。`;
}
...
function refreshMarkers(){
  ...
  ITEMS[activeTab].forEach(it => {
    ...
    L.circleMarker([it.lat, it.lng], {
      radius: inside ? 6 : 4, weight: inside ? 2 : 1.5, color:"#33302B",
      fillColor: inside ? "#B3566E" : "#FFFFFF", fillOpacity: inside ? .95 : .55
    }).bindPopup(`<b>${esc(it.title)}</b><br>${esc(place)}<br>${esc(it.date || "")}<br>中心から ${d.toFixed(1)}km`)
      .addTo(markerLayer);
  });
}
...
function startView(){
  const st = curState();
  if(st.mapArea) return {center:[st.mapArea.lat, st.mapArea.lng], radiusKm:st.mapArea.radiusKm};
  if(lastView) return lastView;
  if(st.userLoc) return {center:[st.userLoc.lat, st.userLoc.lng], radiusKm:15};
...
function initMap(){
  if(!map){
    map = L.map("mapCanvas", {
      ...
      zoomSnap:0, zoomDelta:0.5,          // ピンチの拡大率をそのまま半径に反映させる
      minZoom:MAP_MIN_ZOOM, maxZoom:MAP_MAX_ZOOM
    }).setView(DEFAULT_CENTER, 10);
    L.tileLayer(TILE_URL, {minZoom:MAP_MIN_ZOOM, maxZoom:MAP_MAX_ZOOM, attribution:TILE_ATTR}).addTo(map);
  ...
  const zoom = view.zoom != null ? view.zoom : zoomForRadius(view.center[0], view.radiusKm);
...
export function openMapSheet(){
  ...
  el.close.focus();
  ensureLeaflet().then(
    () => { try{ initMap(); } catch(e){ showMapError(`地図を表示できませんでした（${e.message}）。`); } },
    err => showMapError(`${err.message}。通信環境を確認して開きなおすか、下のエリアから絞り込んでください。`)
  );
}
...
export function closeMapSheet(){
  ...
  const back = (mapReturnFocus && mapReturnFocus.offsetParent !== null)
    ? mapReturnFocus : document.getElementById("sortLocBtn");
...
function onMapLocate(){
  ...
  if(pre){ setMapNote(pre); return; }
  const st = curState();
  if(st.userLoc && map){ map.panTo([st.userLoc.lat, st.userLoc.lng]); setMapNote(defaultMapNote()); return; }
  el.locate.disabled = true;
  setMapNote("現在地をさがしています…（許可ダイアログが出たら「許可」を選んでください）");
  getPositionWithRetry(
    ...
    err => { el.locate.disabled = false; setMapNote(geoErrorMessage(err)); }
  );
}
...
const ps = {};
let placeMap = null, placeLayer = null, placeReturnFocus = null;
...
export function initPlaceSheet(){
  ...
  ps.sheet.addEventListener("click", e => { if(e.target === ps.sheet) closePlaceSheet(); });
  ps.sheet.addEventListener("keydown", e => {
    if(e.key === "Escape"){ e.preventDefault(); closePlaceSheet(); return; }
    trapTab(ps.sheet, e);
  });
  window.addEventListener("resize", () => { if(!ps.sheet.hidden && placeMap) placeMap.invalidateSize(); });
}

function initPlaceMap(points){
  if(!placeMap){
    placeMap = L.map("placeMapCanvas", {preferCanvas:true, zoomControl:true}).setView(DEFAULT_CENTER, 10);
  ...
  points.forEach(b => {
    ...
    const popupHtml = `<b>${esc(b.name)}</b><br>${esc(b.area || "")}` +
      (href ? `<br><a href="${esc(href)}" target="_blank" rel="noopener noreferrer">サイトを見る ↗</a>` : "");
    L.circleMarker([b.lat, b.lng], {radius:7, weight:2, color:"#33302B", fillColor:"#B3566E", fillOpacity:.9})
      .bindPopup(popupHtml).addTo(placeLayer);
  });
...
function upcomingAt(name){
  ...
  TAB_ORDER.forEach(key => {
    ITEMS[key].filter(it => venueNames(it).includes(name))
...
function placeFacts(name){
  ...
  if(v){
    return [v.kind ? VENUE_KINDS[v.kind] || v.kind : null,
            v.capacity ? `収容 約${v.capacity.toLocaleString()}人` : null,
            v.area].filter(Boolean).join("　");
  }
...
export function openPlaceSheet(name){
  ...
  ps.loading.textContent = points.length ? "地図を読み込んでいます…" : "この会場の位置情報が未登録です。";
  ps.close.focus();
  if(!points.length) return;
  ensureLeaflet().then(
    () => { try{ initPlaceMap(points); } catch(e){ ps.loading.hidden = false; ps.loading.textContent = `地図を表示できませんでした（${e.message}）。`; } },
    err => { ps.loading.hidden = false; ps.loading.textContent = `${err.message}。通信環境を確認して開きなおしてください。`; }
  );
}
...
export function closePlaceSheet(){
  ...
  document.body.style.overflow = "";
  setBackgroundInert(false, [document.querySelector(".controls")]);
  const back = (placeReturnFocus && placeReturnFocus.offsetParent !== null) ? placeReturnFocus : null;
  if(back && typeof back.focus === "function") back.focus();
