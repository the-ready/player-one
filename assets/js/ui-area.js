...
export function initArea() {
  ...
  el.kinds.addEventListener("click", (e) => {
    ...
    if (!btn) return;
    venueKindFilter =
      venueKindFilter === btn.dataset.kind ? null : btn.dataset.kind;
    buildVenueKinds();
    renderVenueList();
  });
...
export function buildVenueKinds() {
  ...
  if (!kinds) {
    el.kinds.innerHTML = "";
    el.kinds.hidden = true;
    return;
  }
  el.kinds.hidden = false;
  el.kinds.innerHTML = Object.entries(kinds)
    .map(
      ([key, label]) =>
        `<button type="button" class="venue-kind" data-kind="${esc(key)}" aria-pressed="${venueKindFilter === key}">${esc(label)}</button>`,
    )
    .join("");
}
...
export function renderVenueList() {
  ...
  if (place.kinds && venueKindFilter)
    opts = opts.filter((o) => venueKindOf(o.venue) === venueKindFilter);
  if (q) opts = opts.filter((o) => o.venue.toLowerCase().includes(q));
  opts.sort(
    (a, b) => b.count - a.count || a.venue.localeCompare(b.venue, "ja"),
  );
...
export function updateMapAreaStatus() {
  ...
  el.mapStatus.innerHTML = `地図で選んだ範囲（中心から半径 ${fmtKm(st.mapArea.radiusKm)}）でしぼりこみ中。<button type="button" id="clearMapArea">範囲を解除</button>`;
...
export function geoPrecheckError() {
  if (!("geolocation" in navigator))
    return "このブラウザは位置情報に対応していません。エリアから選んでください。";
...
function showLocateReady() {
  ...
  el.locateMsg.querySelector("#clearLoc").addEventListener("click", () => {
    ...
    el.locateMsg.className = "locate-msg";
    el.locateMsg.textContent = "";
    refreshNow({ push: true });
  });
}
