...
import { esc, safeUrl, highlight, fmtDateWd, fmtWhen, haversineKm } from "./util.js";
import { LINEUP_VISIBLE, GENRE_TABLE_BY_TAB, SCREENING_TYPES, LIVE_TYPES,
         tableMeta, statusMeta, catMeta, venueNames } from "./config.js";
import { venueMeta, isKnownChain, theaterMeta } from "./data.js";
import { onsaleState, onsaleInDays, deadlineDays, isNewlyAnnounced,
         seriesSiblings, seriesHighlights } from "./filters.js";
...
const ctaLabel = (source, cta) => source ? `${source}${cta.suffix}` : cta.fallback;
const ctaAria  = (title, source, cta) =>
  source ? `${title}を${source}${cta.ariaWith}` : `${title}の${cta.ariaWithout}`;
...
const priceTxtHtml = p => (p && !NO_PRICE.includes(p.trim())) ? `<span class="price-txt">${esc(p)}</span>` : "";
...
function hasPlaceInfo(name, it){
  return isKnownChain(name) || !!venueMeta(name) || !!theaterMeta(name) ||
...
function venueCapHtml(it){
  ...
  const parts = [kindLabel, v.capacity ? `約${v.capacity.toLocaleString()}人` : null].filter(Boolean);
  return parts.length ? `<br><span class="venue-cap">${esc(parts.join("／"))}</span>` : "";
}
// 循環importを避けるため、ラベルだけここに置く（config.js の VENUE_KINDS と同じ内容）。
const VENUE_KIND_LABEL = {
  dome:"ドーム・スタジアム", arena:"アリーナ", hall:"ホール",
  livehouse:"ライブハウス", outdoor:"野外・フェス会場"
};
...
function ticketBlockHtml(tab, it){
  ...
  if(onsale === "unknown" && !it.limitedSale && !hi.limited && !hi.before) return "";
  ...
  if(onsale === "before"){
    ...
    if(when) rows.push(`<span class="ti-row"><b>${esc(when)}</b> 受付開始${n != null ? `（${n === 0 ? "本日" : `あと${n}日`}）` : ""}</span>`);
    const endWhen = fmtWhen(it.onsaleEnd, it.onsaleEndTime);
    if(endWhen) rows.push(`<span class="ti-row ti-sub">締切 ${esc(endWhen)}</span>`);
  } else if(onsale === "open"){
    const startWhen = fmtWhen(it.onsaleStart, it.onsaleStartTime);
    if(startWhen) rows.push(`<span class="ti-row ti-sub">${esc(startWhen)} より受付中</span>`);
    const d = deadlineDays(it);
    const endWhen = fmtWhen(it.onsaleEnd, it.onsaleEndTime);
    if(endWhen) rows.push(`<span class="ti-row">締切 <b>${esc(endWhen)}</b>${d != null ? `（${d === 0 ? "本日まで" : `あと${d}日`}）` : ""}</span>`);
  }
  if(it.limitedSale) rows.push(`<span class="ti-row ti-limited">限定・追加販売：${esc(it.limitedSale)}</span>`);

  // シリーズ内の他会場にだけ出ている枠を知らせる。**この行に無いものだけ**を出す。
  // 自分も同じ状態なのに「他N件が発売前」と書くと、他にだけ何かあるように読めてしまう。
  const notes = [];
  if(hi.limited && !it.limitedSale) notes.push(`他${hi.limited}件に限定・追加販売あり`);
...
function seriesOthersHtml(tab, it){
  ...
  return `<div class="series-others">
      ...
      <ul>${others.map(o => {
        ...
        const tag = s === "unknown" ? "" : `<span class="ts-tag ${s}">${esc(words[s])}</span>`;
        const lim = o.limitedSale ? `<span class="ts-tag limited">限定・追加販売</span>` : "";
...
function icsEscape(s){
  return String(s == null ? "" : s).replace(/[\\;,]/g, m => "\\" + m).replace(/\r?\n/g, "\\n");
}
...
export function buildIcs(it){
  ...
  const dtEnd = icsDate(`${end.getFullYear()}-${String(end.getMonth()+1).padStart(2,"0")}-${String(end.getDate()).padStart(2,"0")}`);
  const where = [...venueNames(it), it.area].filter(Boolean).join(" ");
  const url = safeUrl(it.url || it.officialUrl) || "";
  const stamp = new Date().toISOString().replace(/[-:]/g, "").replace(/\.\d+/, "");
  return [
    "BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//eventboard//JP","CALSCALE:GREGORIAN",
    "BEGIN:VEVENT",
...
export function gcalUrl(it){
  ...
  const p = new URLSearchParams({
    ...
    details:[it.desc, safeUrl(it.url || it.officialUrl)].filter(Boolean).join("\n"),
...
export function cardHtml(tab, it, st, terms){
  ...
  const detailBody = [
    ...
    it.note ? `<p class="coupon-note"><span class="coupon-tag">注意</span>${esc(it.note)}</p>` : "",
    it.posterUrl && it.posterSource ? `<p class="poster-credit">ポスター画像出典：${esc(it.posterSource)}</p>` : ""
  ].join("");
  ...
  <article class="card ${tab.cardClass}" data-key="${esc(it.key)}">
    ...
    <div class="${tab.bodyClass}">
      ...
      ${href ? `<a class="go-btn${tab.key === "movie" ? " ticket-btn" : ""}" href="${esc(href)}" target="_blank" rel="noopener noreferrer" aria-label="${esc(ctaAria(it.title, it.source, tab.cta))}">${
        tab.key === "movie"
