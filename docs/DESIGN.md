...
> **`~/.claude/skills/` への複製は運用していない**（2026-08-19時点）。**`.claude/skills/` が唯一の正本**であり、週次ルーチンは `/kanto-*-collector` のスラッシュコマンド名による解決に頼らず、`.claude/routines/event.txt` の指示でこのパスを直接読む（第9.1.5節）。複製を運用に戻す場合は、CLAUDE.md の9か所チェックリストに「複製の手動同期」を追加すること——古い複製が呼ばれると、列の規則だけが先週のままになる。
...
| `PreToolUse(Agent)`        | `.claude/hooks/agent-guard.sh`  | ルーチン中、サブエージェントの**背景起動**を拒否する（第9.3.2節）                                                                           |
...
伸ばせる手段でもあるため禁じない。**縛るのは起動の形だけ**としている。
...
.claude/
  ...
  hooks/                      規則を決定論的に守らせるフック（第9.1.5節）
    agent-guard.sh            ルーチン中のサブエージェントの背景起動を拒否（PreToolUse:Agent）
  ...
  settings.json               権限とフックの登録（フックの中身は hooks/ と tools/fetch_gate.py・tools/budget.py）
tools/
