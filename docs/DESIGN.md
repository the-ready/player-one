...
開催状況は `本日開催`/`まもなく開催`/`開催中`/`公演予定`/`終了` で、**CSVではなく表示のたびに日付から決まる**（第3.9節）。**価格比較の列（`price_official` 等）は持たない**（第12.6節）。**画像URLの列も持たない**（第7.1節）。
...
この構成にしているのは、`price` の原文が「一般2,300円/大学生1,300円/高校生900円、前売各200円引き」のように**折り返して2〜3行を占めることが多く**（実データでイベント81件・映画72件・ライブ86件）、カードの一覧性を最も削っていたためである。比較の結論（最安値）も「くわしく」の中にしかないため、表面の原文だけを見た人が**この画面で一番安い経路を知らないまま**判断してしまう状態を避けるためでもある。
...
**3つの収集スキルはすべてリポジトリ内の `.claude/skills/` に置く。** リポジトリ外にあると、CSVの列や構成の変更がスキル側へ自動で伝わらないためである。`~/.claude/skills/` にはそのコピーを置いて `/kanto-*-collector` として呼び出せるようにする。

> **重要**: `.claude/skills/` が正本で、`~/.claude/skills/` はそのコピーである。**同期は自動ではないので、CSVの列や書き出し先を変えたら両方を手で更新すること。**

**同期漏れは、最も気づきにくい壊れ方をする。** 古い複製が呼ばれると、書き出し先がルート直下の `events.csv`（`data/` の外）になる。ダッシュボードは `data/` 配下しか読まず別パスへのフォールバックも持たない（第3.2節）ので、**収集は成功したように見えて画面は先週のまま**になり、`claude-routine.sh` は `data/` と `docs/` しかコミット対象にしないので警告1行を残して正常終了してしまう。指示文で注意するだけでは防げないので、仕組みの側に2つ置いてある。

| 対策                                                                   | 効果                                                                      |
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `validate_data.py` がルート直下の `events.csv` 等を **ERROR** で落とす | 迷子になった週をコミットさせない（＝「古いが正しい」状態が守られる）      |
| `.claude/routines/event.txt` が手順書を**パスで読ませる**              | `/kanto-...` の名前解決に任せず、リポジトリ内の正本を読むことを確実にする |
...
| 対象地域     | イベント・映画は関東1都6県／**ライブは関東1都6県＋隣接5県**（山梨・静岡・長野・新潟・福島） | エリア絞り込みの主対象と一致させる。ライブだけ広いのは、関東から日帰りできる圏内に動員の大きい会場・フェスが実在するため（下記）。隣接県は `pref="other"` で登録し、県名は `area` に書く                      |
...
| 取得の可否   | **`robots.txt` が `Allow` しているURLだけを、間隔を空けて取得する**                         | サイト単位の可否では、トップを開放したまま閉じている個別のパス（予約・API・検索）を踏むため、**URL単位**で判定するものとしている。判定と待機は取得の直前に機械が行う（`COLLECTION-PROTOCOL.md` 6.5.4〜6.5.5） |
...
**イベント・映画は関東1都6県のまま**にしている。`spots.csv` と `theaters.csv` には関東以外の登録が1件も無く、広げる動機が無いためである。3タブで揃えることより、**実態に無い範囲を宣言しないこと**を優先する。
...
.claude/
  skills/
    kanto-event-collector/    イベント収集スキル ← **正本**（~/.claude/skills/ のコピーは手動同期）
...
tools/
  ...
  smoke_test.mjs              画面のスモークテスト（Playwright。3タブ・絞り込み・URL・キーボード）
...
| ライブの東京比率が目標（50%以下）を超えている              | 地方会場は公式サイトを読めないと拾いづらく、検索要約だけでは裏が取れない                                        | 収集スキルのステップ1で都県ごとに名簿を回す。`--pref other` の指定漏れにも注意（第9.2節）                                               |
...
| 収集スキルの複製は手動同期                                 | `~/.claude/skills/` は自動では追随しない                                                                        | 第9.1節の2つの網（ルート直下CSVのERROR、手順書をパスで読ませる）で、被害が出る前に気づく                                                |
...
第12.11節の検証済みリンクを428組ぶん毎週引き直すのは、収集の現実に合わない。かといって空欄にすると、**この層の主目的（聴いてから行くか決める）がほとんどの行で果たせない**。そこで2層にした。

| 層                     | 出どころ                                          | 見た目 |
| ---------------------- | ------------------------------------------------- | ------ |
| `apple_music_url` あり | 収集時に iTunes Search API で実在を確認した       | ♪ 塗り |
