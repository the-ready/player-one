...
| `tour_id` | 同じツアー・同じフェスの行に共通の値。「このツアーの他の公演」を引くキー（第13.5節） |
| `artists` | 出演者。`\|` 区切り。**検索対象であり、カード上部のラインナップ表示の材料でもある**（第13.5節） |
...
| `limited_sale` | その公演にだけ出ている枠（見切れ席・追加席・公式リセール・当日券）。**ツアーでは公演ごとに違う**（第13.5節） |
