...
- **終日を既定にし、時刻指定はいつでも選べる第二の選択肢として並べている。** このサイトの日程データは会期はあっても時刻が未定の行のほうが多く、時刻を先に聞くと決まっていない利用者を迷わせる。`preset-chip` と同じ見た目のトグル（終日／時刻を指定）にし、`start_time` があれば時刻指定側を初期値にする以外は終日を初期値にしている
