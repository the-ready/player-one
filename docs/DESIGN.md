...
#### 4.2.1 例外：検索語（`q`）だけは3タブで共有する

検索窓はDOM上に1つしか無く、他の絞り込み（ファセット・日程・エリア）とは食い違い方が違うため、`q` だけをタブ分割の外に出している。

- **タブをまたいでも入力欄の中身が変わらないほうが自然なため、共有としている。** 「Vaundyで検索したのにタブを移ったら入力欄が空になった」という体験は、「絞り込みが引き継がれて0件になる」（上の状態を分ける理由）より混乱を招く。ファセット・日程・エリアは1つ立てるだけで別タブの母集団がまるごと変わりうるが、検索語1語だけでそこまで極端な食い違いは起きにくい
- **共有した検索語が他のタブに何件当たるかは、タブボタンの件数バッジで示している。** 検索中だけタブに数字が付き（`イベント 3`）、押せばその中身が見える。バッジの件数は「そのタブ自身の `matchesFilters`（＝ファセット・日程・エリアはそのタブの状態のまま）」で数えているので、**タブを開き直したときに実際に見える枚数と必ず一致する**——検索語だけを別ロジックで数えると、facet や日程で絞っている最中にバッジと実際の枚数が食い違う
- 実装は `state.js` が1つの変数を全タブの `state.q` から `Object.defineProperty` の getter/setter で参照する形にしており、`filters.js`・`render.js` 側は `st.q` を読み書きするだけで共有を意識しなくてよい
...
**土曜・日曜・祝日の色分け**：
カレンダーは日付を選ぶための道具なので、休日かどうかが一目で分かったほうが「今週末」「祝日を挟む連休」の判断が速い。土日は曜日の計算だけで判定できるが、祝日は法律（暦要項）で決まる値なので計算では出せない。そこで [内閣府「国民の祝日について」](https://www8.cao.go.jp/chosei/shukujitsu/syukujitsu.csv) が配布する祝日データを `data/holidays.csv` として取り込み、`assets/js/ui-calendar.js` がそれを土日と同じ扱いで色分けする（振替休日・国民の休日もこのデータに含まれるため、別ロジックは要らない）。出典データは[公共データ利用規約(第1.0版)](https://data.e-gov.go.jp/info/ja/terms)（CC-BY相当・出典表示必須・加工した旨の明記が必要）のもとで日付をISO表記へ変換して使っている。

祝日は法改正のとき以外ほぼ動かないため、`theaters.csv` / `venues.csv` と同じ「準静的なマスター」として扱い、週次の収集スキルには含めず `tools/update_holidays.py` を人が随時実行してコミットする形にしている（第9.1節の週次サイクルの対象外）。この列は `data/*.csv` の9か所ルール（CLAUDE.md）にも意図的に含めていない——祝日はイベント／映画／ライブのどのスキーマにも属さない、表示専用の判定テーブルだからである。

カード側の日付表記（`2026.9.21(月)`）は今回は対象にしていない。祝日データを持たないために `(月・祝)` が書けないという制約は残っており、対応する場合は `assets/js/util.js` の `fmtDateWd` を3タブ共通で拡張する必要がある（第3.9.4節）。
...
### 5.10 カレンダーに追加 — 渡す前に日時を選ばせる

会期のある催し（美術展など、時に数ヶ月続く）をそのまま終日イベントとして端末やGoogleカレンダーへ渡すと、実際に行く1日ではなく会期全体が予定表に載ってしまう。また `.ics` を端末へ渡すだけでは、OS側の画面が確認とAdd/Cancelしか持たず日時を直せないことが多い。そのため、**「端末のカレンダーに追加」「Googleカレンダーに追加」のどちらを選んでも、渡す前に日付・時刻を選ぶシート（`ui-caladd.js`）を必ず経由させる**設計にしている。

- **Googleカレンダー側も同じシートを通す。** Google の作成画面自体にも編集機能はあるが、渡す時点で正しい候補になっているほうが手間が少ない。端末側だけ画面を挟むと「片方だけ聞かれる」非対称が生まれるため、2つの導線を対称に保っている
- **終日か時刻指定かは選ばせず、開始時刻が入っているかどうかで決める。** 入力欄は「日付・開始（任意）・終了（任意）」の3つだけで、時刻を空のままにすれば終日の予定になる。モードを選ぶトグルを別に置くと、利用者は同じことを2回（モードの選択と時刻の入力）伝える必要があり、しかも「時刻を指定を選んだのに時刻が空」という矛盾した状態まで作れてしまう。`start_time` を持つ行では開始欄を初期値として埋めておくので、時刻が分かっている催しは何も操作せずそのまま時刻つきで追加できる
- **終了だけが入力された状態は、黙って終日に落とさず開始の入力を促す。** 時刻を入れる意図があったのに片方を取りこぼした入力であり、そのまま終日として登録すると利用者の意図と違う予定が静かに残るためである
- **中身が入力欄3つのフォーム1枚なので、シートは全高ではなく内容の高さだけを取る。** 地図・出演者シートと同じ全高レイアウトにすると下半分が空白のまま残り、`.map-sheet-foot` の区切り線だけが宙に浮いて見える。このシートだけボトムシートとして高さを内容に合わせ、区切り線も持たせない
- **日付は自由に選ばせ、会期の範囲に制限しない。** 会期中のどの1日に行くかは利用者だけが知っており、前後泊の都合などで範囲外の日を選ぶ理由も正当にありうる
- **初期値の起点は「今日」ではなく「明日」に置く**（`schedule.js` の `suggestedVisitDate()`）。この画面を開くのはこれから行く日を決めるためで、すでに始まっている今日を提案しても選び直しになるためである。会期の形ごとに次の4つに分けている

  | 行の形              | 初期値                                 |
  | ------------------- | -------------------------------------- |
  | 飛び日程（`dates`） | 明日以降でいちばん近い開催日           |
  | 会期が始まっている  | 明日（会期末がそれより前なら末日）     |
  | これから始まる      | 初日                                   |
  | すでに終わった      | 末日（過去の行でも空欄のまま開かない） |

  終了日が空の行は「初日と同じ1日だけ」ではなく「◯月◯日〜（終わり未定）」を意味する（`util.js` の `fmtSpan` と同じ読み方）ので、**判定で `end_date` を `start_date` で埋めない**。埋めると会期中の行が初日に張り付く。この分岐は画面を開かないと値が見えず回帰に気づけないため、`tools/caladd_test.mjs` で純粋関数として固定してある

- **終了時刻が空、または開始以前のときは開始の2時間後を補う。** ライブ・イベントの目安の長さとして妥当で、終日でも時刻指定でも終了が開始より前という壊れた区間をカレンダーに残さない
- **時刻を指定した予定は `TZID`/`VTIMEZONE` を持たず、単純なUTC（`Z`終端）に変換して書く。** 日本は通年UTC+9でDSTを持たないため、時刻から9時間引くだけで常に正しくなる。`VTIMEZONE` を自前で組む・タイムゾーンデータベースを保守する手間は、`.ics` を生成するだけの用途には見合わない
- **override（選んだ日時）を渡さない呼び出しは、これまでどおり行の会期全体を終日イベントとして扱う。** `buildIcs()`/`gcalUrl()`（`cards.js`）はカード描画時に「カレンダーボタンを出すかどうか」の判定にも使われており（`cards.js` の `cardHtml()`）、その経路までシートに依存させる理由が無いためである
...
- **雲の配置は、Webフォントの読み込み完了時とコンテナのリサイズ時にも引き直す。** `layoutClouds()` が避けるのは呼ばれた瞬間の文字の実測位置でしかなく、そのあとWebフォント（Zen Maru Gothic 等）の読み込みで文字幅が変わったり、画面幅・端末の向きの変化で折り返しが変わったりすると回避が古いまま残ってしまうため、`document.fonts.ready` の解決時と `ResizeObserver` の発火時（デバウンス）に、動的壁紙が有効なときだけ同じ `cloudSeed` で再配置している（形・個数の乱数は変えず、避ける位置だけ更新する）
- **雲を避ける文字の矩形には、雲のドリフトアニメーションの最大振幅（26px）ぶんの左右マージンを足している。** 雲は配置後も横に最大26pxまで揺れ動く（`.cloud > svg` の `drift` アニメーション）ため、配置の判定時点で重なっていなくても揺れの端で文字にかかりうる。そのため回避判定側の矩形をあらかじめ振幅ぶん広げている
...
| `PreToolUse(Agent)`        | `.claude/hooks/agent-guard.sh`  | ルーチン中、サブエージェントの**背景起動**・**事前定義していない種類での起動**・**前の波を書き切らない起動**・**残量が線を越えてからの起動**を拒否する（第9.3.2節・第9.3.15節） |
...
#### 読み取りの側にも門を置く —— 1回の `Read` が、以後のターン数ぶん課金される

フックは長らく「外に出るもの」（取得・検索・git・サブエージェントの起動）だけを見ていた。**読み取りには門が無かった。** これは、読み取りが安いという前提に立っている——その前提が成り立たないものとしている。

`Read` の既定の上限は**行数**（2000行）で、1行の長さは見ない。`data/events.csv` は406行しかないので、上限に一度も触れないまま**178,387文字が丸ごと文脈に入る**。しかも文脈は毎ターン送り直されるので、**1回の `Read` が以後のターン数だけ課金される**。親が残り60ターン回るなら、文脈再送は5〜7M増える——撤退の線（40M）の2割近くを、「CSVが更新されたか確認する」ためだけに使うことになる。

この経路は既に一度踏んでいる。2026-09-02 の events は、`skill_brief.py` の出力が Bash の出力上限を越えて `tool-results/` のファイルに落ち、**親がそれを丸ごと `Read` した**（`docs/routine-postmortems.md`）。

止める基準を「大きいこと」にはしていない。子が読む抜粋（`temp/brief-*.md`）は約4万文字あるが、**あれは読まれるために作ったファイル**で、塞げば子が規則を1つも受け取れなくなる。基準は**同じ情報をもっと安く取れる道具がこのリポジトリにあるか**である。

| 読もうとしているもの               | 代わりに使うもの                                         |
| ---------------------------------- | -------------------------------------------------------- |
| `.claude/skills/kanto-*/SKILL.md`  | `tools/skill_brief.py` の抜粋／`offset` 付きで該当節だけ |
| `data/*.csv`・`data/.prev/*.csv`   | `tools/prev_rows.py --worklist` / `--uid`、`wc -l`       |
| `data/{spots,venues,theaters}.csv` | `tools/roster.py --list --urls`                          |
| `temp/rows-*.jsonl`                | `tools/append_rows.py` に流す（件数は `wc -l`）          |
| `*/tool-results/*`                 | 出力を絞るコマンドで取り直す                             |

**代替の道具が無ければ通す。** `skill_brief.py` が壊れている回にまで全文へのフォールバックを塞ぐと、親が手順書に一度も到達できない。読み取りを塞ぐゲートは、止めすぎの害が止めなさすぎの害を上回る数少ない位置にあるので、逃げ道を明示的に持たせている（`tools/read_gate_test.py` がこの逃げ道を固定する）。

#### 子が終わってから親が受け取るまでの間にも、門が無かった

「結果は `temp/rows-<波の名前>.jsonl` に書き、返答にはパスと件数だけ」という契約は各SKILL.mdにあり、**そこだけ散文のままだった**。背景起動は `agent-guard.sh`、波の書き切りは `wave_gate.py`、抜粋の受け渡しも `agent-guard.sh` が見ているが、子の**返答の中身**は誰も見ていない。

行を返答に載せると、同じ行を3回払う——子が書き（出力）、親の文脈に入り（入力。以後、親のターン数だけ再送）、親が書き直す（出力）。2026-08-26 の lives は88行でこれをやり、`03:07:48 → 03:11:21` の3分半がほぼ純粋な出力生成に消えている。
...
`docs/skill-feedback.md` の 2026-09-05 が「次の波を投げる前に前波の最長ターン数を親に見せる」ことを求めている。これを起動の側（`agent-guard.sh`）には置けない——あちらが数字を見せられるのは**拒否したときだけ**で、拒否は線を越えてからしか起きず、「次の波は小さく割ろう」と決めるのに間に合わない。

`PostToolUse(Agent)` なら、波を受け取った直後・次の分割を決める前に必ず目に入る。伝達には `hookSpecificOutput.additionalContext` を使う（stdout に素で書くとユーザーのトランスクリプトに出るだけで、モデルには届かない）。**止めない。** ここで止めても既に回り終えた子のターン数は減らず、できるのは次の波の分け方を変えることだけである。
...
| 課題                                                                    | 対応                                                                                                                                                             |
| ----------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 前回CSVを読むとコンテキストの2〜3割を失う（実測値は下のコマンドで確認） | `prev_rows.py --worklist` が棚卸しに必要な列だけを出す（削減後の実測値も同様に確認）。全列は uid 指定で引く                                                      |
...
### 9.3.4.1 書き込みは upsert にし、前回分は調査の前に書き戻す

`append_rows.py` は追記専用で、既にCSVにある催しを再収集すると必ず重複行が増えていた。この制約のため、収集手順は「調べ終えてから、残った前回行を書き戻す」順序を取らざるを得ず、**予算が尽きた回では数百行が未確認のまま最後になだれ込んでいた**（2026-09-26 の events 収集は349行中304行がこれに当たり、実質的に調べられたのは28行だった）。

そこで書き込みを uid での upsert にし、`--carry-rest --apply` を**調査の前**へ移している。この順序では、CSVは着手直後に先週の完全な状態へ戻り、以降の調査はすべて上積みになる。**打ち切りで失われるのは「今週増やせたはずの分」だけ**で、既に載っているデータは影響を受けない。

置き換えは行ごとで、列ごとの重ね合わせではない。空欄で上書きして消す（料金が無くなった・受付が終わった）という操作を残すためで、「確認できないものは空欄」という規則と噛み合わせている。消えては困る安定した事実（`desc` `cats` `area` `official_url` と会場の事実）は `CARRY_ALWAYS` が受け持ち、それ以外の列が更新で空になった場合は警告に出す。

この順序変更に伴い、`--dispose` は記録するだけでなく**`notfound` 以外の行をCSVから取り除く**。以前は `--init` がCSVを空にしていたため記録だけで足りたが、先に書き戻す以上、記録だけでは終わった催しが公開され続けるためである。

**削除は、処分の記録より先に行う。** 記録が先だと、削除が失敗したとき「記録上は処分済みなのにCSVには残っている」という不整合が生じる——記録は追記専用で取り消せないため、再実行しない限りその週はこの不整合を抱えたまま終わる。削除を先にすれば、削除が失敗した時点で明確なエラーとともに処理が止まり、記録はまだ書かれていないので、呼び出し側は同じ入力をそのまま再実行するだけで安全にやり直せる。

**表記ゆれの検出は、この順序でむしろ重要になる。** 催しの表記が変わると uid が変わるため、旧uidの持ち越し行と新uidの新規行が**両方ともCSVに残る**。`diff_data.py` の `[表記が変わった可能性]` を確かめ、同じ催しなら `renamed` として処分する工程は省略できない。

### 9.3.4.2 renamed 検知は、carry-rest が書き戻した行も「消えた」側に含める

第9.3.4.1節の順序（carry-rest を調査の前に置く）には、当初見落としていた欠陥があった。
`diff_data.py` の `[表記が変わった可能性]` は「前回にあって今回のCSVに無い行」を入力に
取るが、carry-rest が着手直後に前回行を全部書き戻すため、**表記が変わった行の旧uid行は
物理的に「消えて」いない**——調査が実際にその行を再確認するまでCSVに残り続ける。結果、
`renamed` を記録するきっかけ自体が発生せず、旧uid行と新uid行が永久に二重掲載される。
2026-08-29 の事故（`[表記が変わった可能性]` の見落としで8件二重掲載）と同じ結果を、
見落としようのない形——検出の仕組み自体が動いていない形で再発させるところだった
（2026-09-27 に実データで確認・修正）。

塞ぎ方は、`renamed` 検知（`fuzzy_pairs` への入力）だけ「消えた」の定義を広げることである。
「物理的に消えた行」に加えて、「carry-rest が書き戻したまま、まだ一度も能動的に
再確認されていない行」も候補にする。判定は `id`・受付欄（`CARRY_REST_CLEAR`）・
`lineup_id` を除く全列が前回値と完全一致するかで行う——1列でも変わっていれば、
その回に誰かが能動的に触ったとみなして対象から外す。`gone` 自体（消滅の報告・
ERROR判定・`is_noop` の材料）は今までどおり物理的な不在だけで決め、広げるのは
表記ゆれの候補探しだけに絞ってある。

### 9.3.4.3 バッチ内の重複マージは、CARRY_NEVER列も含めて警告する

carry-rest は前回CSVの全行を1回のバッチで `append_rows.py` に渡す。バッチの中に
同じuidの行が複数（前回CSVに残っていた重複）あると、upsert は後の行で前の行を
上書きする。desc等のCARRY_ALWAYS列は同じ波の中の持ち越し（`seen_in_batch`）が
既に補うが、それ以外の列（price・url・note等）は補われないまま「後の行が勝つ」ため、
前の行の値が完全に無警告で消えうる（2026-09-27 に実データで確認。この週の実データ
自体には実害が無かった——後で現れる行のほうが情報が多いという、この重複の
発生経緯に由来する偶然の並びによる）。

既存の列消失警告（`prepare_records` が保存済みCSVとの比較で行うもの）は
CARRY_NEVER列を対象から除いている——「今週は確認できなかった」という正当な
更新まで警告すると鳴りすぎるためである。だがバッチ内の重複マージにはその
「今週の確認」という判断が介在しない、純粋に機械的な事故なので、CARRY_NEVER列も
含めて見る——同じ理由で `_no_carry` を明示した意図的な空欄化も同様に警告するが、
これは既存の（保存済みCSVとの比較による）警告と同じ、既に受け入れている
トレードオフである。
...
### 9.3.6 desc は「書けた」で満足せず、劣化していないかも機械が見る

`desc` は日付・料金と違って「持ち越し禁止」でも「持ち越し必須」でもない中間の列で、`_carry` に明示したときだけ持ち越す（第9.3節の表）。この設計には見落としがあった——**`_carry` は空欄の列にしか効かない**ため、モデルが何かひとことでも書けば、それだけで前回の値は失われる。

2026-08-30 にHaikuでの実行で見つかった実例では、「ロン・ミュエク」の `desc` が前回の91字（大型新作《マス》を含む…という具体的な説明）から、再確認の回で11字（「彫刻家ロン・ミュエク展」）に書き換わっていた。同種の退行が21件あり、多くは会場の「今月のイベント一覧」のような一覧ページを再確認の情報源にしたときに、そこにある薄い一文がそのまま `desc` を置き換えたものだった。

`desc` の質（100〜150字・目玉が具体的、「`desc` の書き方」章）は散文の基準でしか測れないが、**「前回より大幅に短くなった」という方向だけは機械的に判定できる**。この判定を人の目視やモデルの自己申告に任せると、質の低下がどのゲートもすり抜けたまま毎週積み重なる（`validate_data.py` はヘッダーと構造しか見ておらず、`report_stats.py` の充足率も非空かどうかしか見ていない）ため、2つの機械的な網を置いている。

| 網                 | 検知対象                                                     | 場所                                                  |
| ------------------ | ------------------------------------------------------------ | ----------------------------------------------------- |
| 長さの下限         | `desc` が単独で30字未満（一般名詞1語で終わっているような行） | `validate_data.py`（WARNING・`DESC_MIN_LEN`）         |
| 退行の検知         | 継続行で、今回の `desc` が前回の半分未満に縮んでいる         | `append_rows.py`（追記時にWARNING）                   |
| 今週の新規行の厚み | lives で、今週あらたに書いた行の `desc` が60字以上ある割合   | `report_stats.py`（`FRESH_MIN_LEN`・`--check-fresh`） |

3つ目を足しているのは、**上の2つがどちらも「行ごと」の網で、「その週に書いた行が全体として薄い」を捕まえられなかった**ためである。30字の下限は1行ずつの極端な短さしか見ず、退行の検知は継続行にしか効かない（新規行には比較対象が無い）。実測では lives の `desc` はどの世代も中央値50字前後で、**30字の下限も充足率60%も一度も割らないまま**、「ツアーの5日目」「第26回」のようなタイトルと日程表の言い換えが大半を占めていた。

`report_stats.py` の `FRESH_FLOOR` は非空かどうかしか見ないので、**短い値は空欄と同じに扱う** `FRESH_MIN_LEN` を足して、字数を満たした行だけを「書けた」と数える。値の決め方は他の下限と違う——lives の `desc` は SKILL.md が100〜150字を求めているにもかかわらず、**その水準に達した回が一度も無く、「良い回の実測」から採れない**。そこで字数は目標の手前（60字）に、割合はこれまでの最良の回（42%）をわずかに上回る50%に置いている。新しい書き方に従えば余裕で通り、従来の一行説明なら必ず落ちる位置である。

それでも**中身が具体的かどうかまでは判定できない。** そこは引き続きモデル自身の仕事であり（収集スキルの「`desc` の書き方」章）、機械の網はその手前で「明らかに薄い」ものだけを拾う最終防波堤として置いてある。

### 9.3.7 充足率は「今週あらたに書いた行」で測る

持ち越しが充足率を水増しするため、**全体の充足率では調査が届いていない週を検知できない**。判定は「前回に無い uid の行」に絞って行う（`report_stats.py` の `FRESH_FLOOR`）。

`prev_rows.py --carry-rest` は受付（`onsale_*`）だけを空にして `price_*` を前回値のまま書き戻す。価格は「いつ確認した値か」を `price_checked` で示せるので、この持ち越し自体は正しい（第7.2.3節）。だが結果として、**調べていない料金が「充足している」と数えられる。**

2026-09-02 の events はその形で通っている。

| 層                   |  件数 |    `price` あり |
| -------------------- | ----: | --------------: |
| 全体                 | 455件 |    266件（58%） |
| うち継続（持ち越し） | 365件 |    237件（65%） |
| **うち今週の新規**   |  90件 | **29件（32%）** |

全体で見れば前回の66%から5ポイント落ちただけに見えるが、**今週その料金を実際に見た行はほぼ無い**。同じ回の `price_official` は新規90件中0件だった。

判定は次の3点で構成する。

| どこ                                          | 何を                                                                                                          |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `report_stats.py --check`                     | 都県の網羅性＋新規行の中核列。終了工程でモデル自身が回す（`--allow-short` / `--allow-thin` で承知を明示する） |
| `report_stats.py --check-fresh`               | 新規行の中核列だけ。`verify-data.sh`（Stop フック）が回す                                                     |
| `report_stats.py` の CORE に `price` を含める | 画面に出る料金の列そのもの。これが表に無かったため、低下がどの数字にも出ていなかった                          |

**Stop フックには `--check-fresh` を置き、`--check` は置かない。** ルーチン中のフックは events / lives / movies の3つを回すので、網羅性まで見ると「今週 events を集めている回が、先週のままの lives の都県不足で止まる」。網羅性は「今週その都県を調べたか」の話なので、調べていないデータセットに問う意味が無い。新規行なら、収集していないデータセットは新規0件＝判定対象なしで素通りする。

**コミットの門（`claude-routine.sh`）には置かない。** あちらで落とすと `data/` が巻き戻り、価格が薄いことを理由に**その週の収集がまるごと消える**——薄い週より悪い結果になる。

**Stop フックの「2周目は黙って通す」対象からも外す。** `purge_ended` / `validate_data` / `diff_data` / `run_gate` の4本は claude-routine.sh 側に独立の控えがあるため2周目は素通しでよいが、`report_stats --check-fresh` には控えが無い。控えの無い門を2周目で無条件に通すと、催促が「1回だけ」ではなく「実質0回」に落ちる。埋めるか `--allow-thin` で承知するまで、ターンを終えようとするたびに繰り返し止める（2026-09-22、`tech` カテゴリの誤分類の指摘をきっかけに、この抜け道を塞いだ）。

**その結果、Stop フックが返す理由は2種類になる。** `verify-data.sh` は両者を分けて持ち、文面も分けている。

| 種類                                                   | コミットは                       | 伝える文面                                                     |
| ------------------------------------------------------ | -------------------------------- | -------------------------------------------------------------- |
| `purge_ended` / `validate_data` / `diff_data` が落ちた | **止まる**（`data/` が巻き戻る） | 「このまま終えると今回の収集は失われます」                     |
| `report_stats --check-fresh` が下限割れを検知した      | 止まらない                       | 「この門はコミットを止めません。だからこそ推測で埋めないこと」 |

**分けているのは、嘘の脅しが最悪の逃げ道を作るからである。** 下限割れの回に「収集が失われる」と伝えると、料金を集め直す代わりに**推測で `price` を埋めれば門は通る**——このスキルが最も強く禁じている行為を、こちらから誘発することになる。何が起きるかは種類ごとに正しく書き、そのうえで「推測で埋めるな」を明示する。

### 9.3.8 無人実行のモデルは、親と子で分ける

週次ルーチンの親と子は、求められる能力が違う。**親の成果は「規則を落とさずに指示を組み立てられるか」で決まり、落ちたことは誰にも見えない。** 子の仕事は「渡されたURLを開いて行を書く」で範囲が狭く、`skill_brief.py` の抜粋という手順書も付く。

したがって、**親は Sonnet**としている。曜日→スキル→モデルの対応表（`.claude/skills/weekly-routine/SKILL.md` の `schedule` ブロック）に子のモデルの列を持ち、`claude-routine.sh` が `CLAUDE_CODE_SUBAGENT_MODEL` として渡す。**この環境変数は Agent ツールの `model` 引数より優先される**ので、親が何を指定しても子は表のモデルになる——親の判断に委ねない。

分ける理由は 2026-09-02 の実測にある。当時は親子とも Haiku で、親は抜粋を貼らずに自作の1〜2KBの指示に要約し、そこから料金の列が丸ごと落ちた（第11.8.1節・`docs/COLLECTION-PROTOCOL.md`）。同じ回の最終報告は「打ち切った工程: なし」「クーポン定点観測: 7社各1回確認」と書いているが、プレイガイドへの取得は0回である。**指示の組み立ても報告も、親の側の仕事がまとめて崩れていた。**

**子（events）は、範囲の狭さから Haiku で足りると判断しているが、2026-09-22時点は一時的に Sonnet にしている。** movies・lives の子は既に Sonnet で毎週動いており、実コスト（`total_cost_usd`）が$1.64〜$7.93の幅で同じサブスクリプションの範囲内に収まっている実績がある。events の子をHaikuからSonnetに変えた場合の費用増を確かめるため、movies・lives の実績と同じ土俵で比較できるようにしたのがこの変更の理由であり、Haiku自体の妥当性（子の仕事は範囲が狭く抜粋という手順書も付く）を否定したわけではない。費用面の懸念が無いと分かれば、対応表の子のモデル列を `haiku` に戻す想定でいる。

なお、公開APIの単価表（Sonnet はHaikuのちょうど2倍）から費用を逆算しようとしても、このアカウントの実際の請求経路（サブスクリプション）とは一致しない——2026-09-22の実測では、単価表からの逆算が実際の`total_cost_usd`のおよそ2倍になった。**費用の見積もりは単価表からの計算ではなく、実行後に`total_cost_usd`を直接読むことでしか信頼できない。**

### 9.3.9 「次の波を止める」だけでは、最初の波そのものの暴走を防げない

第9.3.3節の `wave_gate.py` と、`tools/budget.py --gate`（`agent-guard.sh` が呼ぶ）は、
どちらも `Agent` の起動——**次の波を投げる瞬間**——にしか門を置けない。

2026-09-04 の lives 収集は、**最初の波**（フェス調査・東京バッチA・東京バッチB）の
中で、東京の会場に通信障害（DNS不通・SSL証明書不一致・タイムアウト）が連鎖した。
1体が `SUBAGENT_TURN_WARN`（60ターン。第11.6節）の2倍を超える141ターンまで
膨らみ、文脈再送39.4Mに達したが、**「次の波」が一度も投げられていない**ので
`--gate` の門は一度も開く機会を持たなかった。その数分後、アカウントの利用上限で
子・親とも強制終了され、この回のフェスの日割りラインナップ（第12.12節）が
`lives.csv` に `lineup_id` だけ残して `lineups.csv` 側が空、という整合性エラーで
検証に落ち、収集全体が巻き戻った。

**次の波を止める門は、最初の波そのものが線を越えるという壊れ方には効かない。**
そこで `tools/budget.py` に `gate_fetch()` を足し、`fetch_page.py`（Bash経由）と
`WebSearch` の**呼び出しそのもの**を門にした（`.claude/hooks/fetch-budget-guard.sh`、
`PreToolUse(Bash|WebSearch)`）。この2つは子から見ても「次の1回」であり、親が
波の帰りを待って停止している間でも、動いている子自身が取得のたびに通る。

線は撤退（`CACHE_READ_RETREAT`・40M）だけを見る。`--gate` の25M（新しい波を
止める）は流用しない——25M〜40Mの間は「動いている波は受け取って書き切る」設計
（第9.3.1節）で、その間の取得まで止めると波を書き切れなくなるためである。

### 9.3.10 計器は、自分のセッションを同定できないなら黙る

第9.3.1節のトークン計測は、セッションの記録（`~/.claude/projects/<slug>/<session-id>.jsonl`）
を読む。以前はIDで引けないときのフォールバックとして「同じプロジェクトでいちばん
新しい記録」を今のセッションとみなしていた。理由づけは「週次ルーチンはロックで
同時実行を禁じているので取り違えは起きない」だったが、**そのロックが防ぐのは
ルーチン同士の同時実行だけで、同じリポジトリで人が対話セッションを開いている
場合は防げない。**

2026-09-04 20:50 の lives 収集がこれを踏んだ。`weekly-routine/SKILL.md` は起動時に
`!` 埋め込みで `budget.py --report` を実行する——**セッション開始の最初期で、自分の
記録がまだディスクに無い瞬間**である。フォールバックは、同じ時刻に動いていた対話
セッション（44.8M）を拾い、「文脈再送44.8M。**撤退の手順に入ってください**」を
モデルの文脈の先頭に載せた。モデルはそれに従い、サブエージェントを1体も起動せず、
検索0回・取得0回・9分で終えている。

**他人の数字を返すくらいなら「分からない」と答える方針としている。** 分からない
ときの倒し方は既に全ての門で決まっていて（`--gate` は 2 を返して通す、`--report` は
トークンの節を出さない）、そちらは計測できないことを理由に収集を止めない。
取り違えだけが、**収集を止める方向に嘘をつける**——線を越えたと誤って告げる。

ただし黙るときは「未計測」と明示する。省略は「トークンの節が無い＝0だった」と
読めてしまい、第8.7.1節（計器に載っていない資源が実行を止めていた）の再発になる。

#### 取り違えは、フォールバックを消しても環境変数から入ってくる

フォールバックを廃止しても**同じ事故に至る経路がもう1本残っていた**。Claude Code は
自分の記録の場所を示すために `CLAUDE_CODE_SESSION_ID` を export する。**対話
セッションの中から `claude-routine.sh` を起動すると、子の `claude` がそれをそのまま
受け継ぐ**ので、新しいセッションが起動元の記録を自分のものだと名乗る。ID で引く
実装は、引けてしまうぶんフォールバックより静かに間違える。

実測では、対話セッションから起動した回が起動元の 38.4M を自分の消費として読み、
`agent-guard.sh` が最初の波を「25M の線を越えている」として拒否した——2026-09-04
20:50 とまったく同じ形で、原因だけが違う。そのため `claude-routine.sh` は起動前に
...
第9.3.1節以降の門は、`wave_gate.py`・`agent-guard.sh`・`fetch-budget-guard.sh` を
含めてすべて**使いすぎを止める天井**である。2026-09-04 20:50 の実行が示したのは、
**調べなさすぎを止める床がどこにも無い**ことだった。あの回は上の誤った撤退指示に
従って一度も外を見ず、同じ日の失敗回が `temp/` に残したJSONLを回収して追記し、
終了工程だけを通した。それでも既存の検証はすべて通っている。

| 門                              | なぜ通ったか                                        |
| ------------------------------- | --------------------------------------------------- |
| `validate_data.py`              | 行の形は壊れていない（ERROR 0）                     |
| `diff_data.py`（空回り検知）    | 回収した77行が「新規」に数えられる                  |
| `report_stats.py --check-fresh` | 当時 `FRESH_FLOOR` が events にしか定義されていない |

結果、先週の87行と中途半端な77行（`onsale_start`/`onsale_end` は0%）が「週次データ
更新」としてコミット・push された。そこで床を2つ置いている。

- **`tools/run_gate.py`** —— 判断ではなく事実だけを見る。**検索も取得も0回**の回と、
  **`append_rows.py <ds> --init`（収集の開始点）を通っていない**回を落とす。前者は
  「外部の情報源を1度も見ていない」であり、どんな事情があっても収集の回ではない。
  後者は、前回のCSVに追記しただけの回が**先週の行を今週の行としてコミットし、
  `data/.prev/` も更新されないので翌週の差分の基準までずらす**ためである。
  撤退した回を罰しはしない——撤退はそこまでに取得を行っているので通る。
  `Stop` フック（その場で直させる）と `claude-routine.sh`（コミットさせない）の
  両方から呼ぶ。判定できないとき（記録が無い・古い）は通す
- **`report_stats.py` の `FRESH_FLOOR` を3スキル分に広げる** —— 「今週あらたに書いた行」
  の中核列に下限を置く。値は git 履歴の実測（良い回と崩れた回）から採り、良い回より
  十分低い側に置いてある。**この門はコミットを止めない**（1ターンだけ強く催促する）
  ので、外したときの費用は小さい

### 9.3.12 文脈の固定費は、読み手ごとに分けて持つ

文脈再送（第9.3.1節）は「1ターンの文脈 × ターン数」で積み上がるので、効くのは
**毎ターン再送される固定物**を減らすことである。実測すると、その固定物の大半が
「読み手には要らない部分」だった。

| 固定物                        | 以前                           | いま                                                                            |
| ----------------------------- | ------------------------------ | ------------------------------------------------------------------------------- |
| 親が読む手順書                | SKILL.md 全文                  | 親向けの抜粋（`skill_brief.py <ds> --for parent`）                              |
| 親が持つ前回分の棚卸し        | 全行                           | `--summary`（`prev_rows.py <ds> --summary`）                                    |
| 子が持つ担当分の棚卸し        | 親が指示に**貼る**             | `temp/worklist-*.md` を Read                                                    |
| 子に積み上がる取得結果（1件） | 上限20,000字（未到達＝無制限） | 上限4,000字（根拠は `tools/fetch_page.py` の `DEFAULT_LIMIT` の直上のコメント） |

**親も全文を読んでいた。** 抜粋（`skill_brief.py`）は子のために作ったものだが、
親の仕事は棚卸し・分割・起動・追記・検証であって行を書かない（第11.5節）以上、
列の表・キーの一覧・深掘りの手順は親にも要らない。そこで抜粋を読み手ごとに
2つ持つものとしている（`--for child` / `--for parent`）。**合わせると親の固定費は
大きく減るが、削減幅を固定の数値では書かない**——スキル本文を1行足すだけで動く値
だからである。今の削減幅は次のコマンドで実測する（`--out` を付けると、書いた先の
パスと一緒にstderrへ「抜粋◯◯文字 / 全文◯◯文字」が出る）：

```bash
python3 tools/skill_brief.py events --for parent --out /dev/null
```

**どちらも許可リストにはしない。** 外す側の表だけを持ち、新しい節は既定で両方に
残る——増えたときに壊れる方向を「余分に入ってトークンを食う」側に倒しておく。
2つに増えたことで**どちらからも漏れる**という新しい壊れ方が生まれるので、
`skill_brief_test.py` が「全文の見出しが、子か親のどちらかには必ず在る」ことを
突き合わせている。加えて、**親向けの抜粋にサブエージェント用の前置き
（「`data/` に書かない・`--init` を実行しない」）が混ざらない**ことも固定した
——混ざれば親は自分の中心的な工程を禁止されたと読む。

**取得結果の上限は、実測の分布を見て決めている。** 根拠の数値（何件測ったか・
何%が切られるか・中央値）は `tools/fetch_page.py` の `DEFAULT_LIMIT` の直上の
コメントに書いてあり、ここでは繰り返さない——取得ページの分布は取得のたびに
変わるので、2箇所に書くと片方だけ更新されて食い違う。**トークンのために調査品質を
落とす**のは、この一連の仕組みで最もやってはいけない誤りである（第9.1.5節の
「出ているだけでは守られない」と同じく、削るときも壊れ方の向きを選ぶ）。

### 9.3.13 手順書は、調べる対象と同じ言語で書く

収集スキル（`.claude/skills/*/SKILL.md`）の散文は日本語で書くものとしている。
扱う対象——会場名・作品名・日本語サイトの `robots.txt`・著作権法47条の5・
CSVへ書き出す `desc`——がすべて日本語であり、**入力の言語はモデルの表層の生成
だけでなく内部の処理にも使われる**ためである。手順書だけを英語にすると、
手順書と対象のあいだに翻訳が1枚挟まる。

**識別子・コマンド・列名は英語のまま置く。** 日本語の地に英語を埋める向きは
理解を落とさないが、逆向き（英語の地に日本語の固有名詞を挿す）は落とす。
全面英語化は後者の形になるので採らない。ASCII（ファイル名・コマンド・列名）が
`SKILL.md` 全体に占める割合は改訂のたびに動くので数値を固定しない。次のコマンドで
実測する：

```bash
python3 -c "t=open('.claude/skills/kanto-event-collector/SKILL.md',encoding='utf-8').read(); a=sum(1 for c in t if ord(c)<128); print(f'{a:,} / {len(t):,} 字がASCII')"
```

**トークンの差は根拠にならない。** 日英のトークン数の差は、非ラテン文字の
コストを引き下げた版のトークナイザ（Opus 4.7以降）で約1.2倍まで縮んでいる。
英訳で浮くのは総量の2割弱にとどまり、第9.3.12節の抜粋（読み手ごとに分ける）に
よる削減幅より一段小さい——こちらも固定の数値では比べない。削減幅を理由に
言語を選ぶ段階ではない。

**この判断が緩む条件はモデルの規模である。** 指示の言語による追従率の差は
大きいモデルではほぼ消えるが、小さいモデルでは開く。子のモデルは
`weekly-routine` の対応表が決めており（第9.3.8節）、小さいモデルを指定する行が
あり得る（2026-09-22時点は一時的にSonnetだが、対応表の性質上いつでも戻り得る）。
子の追従率が言語に起因して落ちたと疑える事故が `docs/routine-postmortems.md` に
積まれたときに見るのは、**全文の英訳ではなく `skill_brief.py` が子へ渡す抜粋の
密度**である——言語を変えると本文・抜粋・`skill_brief_test.py`・CLAUDE.md の
9か所チェックリストが同時に動くので、戻せる側から先に試す。

**分割して短くする道は採らない。** 公式の進行的開示（`SKILL.md` を目次にして
詳細を参照ファイルへ出す）は、参照先を読むかどうかがモデルの裁量になる。
`skill_brief.py` は `SKILL.md` **1ファイルだけ**を読み、外す節の表だけを持って
抜粋を作る（第9.3.12節）ので、参照ファイルへ出した規則は親向け・子向けの
どちらの抜粋からも黙って消える——この道具が避けるために作られた壊れ方そのもの
である。正本を1ファイルに保ち、部分開示は `Read` のオフセット指定で行う。

### 9.3.14 `description` は英語で書き、日本語の呼び出し語を埋める

スキルの `description` は英語で書くものとしている。この欄は起動時に全スキルの
メタデータが並ぶ場所へ注入され、モデルはそこだけを見てスキルを選ぶので、
他スキルのメタデータと地の言語を揃えるためである。

**ただし日本語の呼び出し語は原文のまま埋める。** 依頼は日本語で来るので、
利用者が実際に打つ語——タブ名（`「イベントボード」`・`ライブ・フェス`）、
`名画座`、`野外上映`、`追加`（公演）、`掲載停止`——を英文の中に置く。日本語を
英文へ挿す向きは第9.3.13節が落ちると書いたほうだが、ここで挿すのは固有名詞と
検索語だけで、規則の文そのものではない。落ちるのは長い散文を混ぜたときで、
索引語を両言語で持つぶんには選択の手がかりが増える。

**上限は1,024文字で、超過は静かな切り詰めとして現れる。** 先に失われるのは
末尾に置いた機能なので、機能を足した回には文字数を数え直す。

```bash
python3 -c "import re,glob;[print(len(re.search(r'(?ms)^description:\s*\"?(.*?)\"?\s*\n(?=\w+:|---)',open(p,encoding='utf-8').read()).group(1)),p) for p in sorted(glob.glob('.claude/skills/*/SKILL.md'))]"
```

**2026-09-16の実測**では `kanto-live-collector` が1,067字で上限を超えていたため、
会場種別の列挙（`domes, arenas, halls, livehouses, outdoor festival sites`）を
落として1,008字にした——会場の種別は `data/venues.csv` の `kind` 列（`dome` /
`arena` / `hall` / `livehouse` / `outdoor`）が持っており、スキルを選ぶ手がかりでは
ないためである。次に語を足す・削るときは、上のコマンドで文字数を数え直すこと
——ここに書いた1,008という値は当時の記録であって、今の値を保証しない。

### 9.3.15 サブエージェントの上限は、指示文ではなくエージェント定義に置く

「1体が回るターン数を60以下に収める」は3つの `SKILL.md` が以前から書いている規則で、
`budget.py --report` も「最長の子◯ターン」を出し、波を受け取るたびに
`wave-report.sh` がその数字を親の文脈へ差し込んでいる。それでも守られなかった。

| 回                | 1体のターン数 | 結果                                                                                      |
| ----------------- | ------------- | ----------------------------------------------------------------------------------------- |
| 2026-09-05 events | 188           | 千葉・茨城・エリア軸探索・クーポン定点観測を一度も実行できず、240件を前回値のまま持ち越し |
| 2026-09-17 movies | 167           | 2波目を起動できず94件を機械的に持ち越し。`official_url` 74%→57%、`outdoor` 2件で下限未達  |

**指示文の目安には、子への強制力が無い。** 親が「60以下に収めろ」と書いても、
その文を読むのは子自身であり、守るかどうかは子の判断に委ねられる。守らなかった
ことが分かるのは、子が返ってきた後——つまり文脈再送を使い切った後である。

強制力を持てる場所は、モデルの外にしかない。`.claude/agents/<name>.md` の
`maxTurns` は Claude Code のクエリループが数えていて、超えた時点で
`max_turns_reached` を返して打ち切る（CLI 2.1.278 の実装を確認）。そこで収集の子を
`.claude/agents/kanto-collector-worker.md` として事前定義し、`maxTurns: 60` を置く方針としている。
この設定はエージェント定義に属するので、`subagent_type` を `general-purpose`
（＝未指定時の既定）のままにすると一切効かない。**定義を置いただけでは守られない**ので、
起動の側にも門を置く（`agent-guard.sh`。第9.1.5節と同じ判断）。

**エージェント定義に `model` は書かない。** 親と子のモデルは `weekly-routine/SKILL.md` の
曜日表が正本で、`claude-routine.sh` がそこから `--model`（親）と
`CLAUDE_CODE_SUBAGENT_MODEL`（子）を決めている。正本を2つにしないためだが、
そもそも**書いても効かない**——CLI 2.1.278 の子モデル解決は

```js
if (process.env.CLAUDE_CODE_SUBAGENT_MODEL) return ...   // 最初にここで返る
if (呼び出し時の model) return ...
let m = 定義の model ?? "inherit";
```

の順で、**環境変数が呼び出し時の引数より先に見られる**（公式ドキュメントが挙げている
「呼び出し時 > 定義 > 環境変数」の順とは違う。実装を優先する）。
つまり無人実行では曜日表の5列目が最終的な決定権を持ち、定義や呼び出し側から
取り違えられる余地が無い。

**同時に、子の書き方を「1件ずつ追記」に変えている。** `maxTurns` が打ち切るのは
応答であってファイルではないため、最後にまとめて書く設計のままだと、打ち切られた
波の調査がまるごと失われる。逐次追記なら、打ち切られてもそこまでが成果として残る。
**この2つは片方だけ入れてはならない。** 上限だけを入れると、これまで完走していた
波が途中で消える側に倒れる。

#### 上限は、波の大きさから逆算して決める

`maxTurns` を入れるだけでは足りない。**担当範囲が60ターンで終わらなければ、
毎回その尾が切り落とされる**からである。切り落とされた尾は次の波か
`--carry-rest` が拾うが、そのぶん余分な波が要り、拾えなければ調査の網羅性が落ちる。

そこで1波に渡す量は、実測の単価から逆算して決めている。

| スキル           | 実測の単価                                             | 1波の上限          | 推定ターン数 | 60に対する余裕 |
| ---------------- | ------------------------------------------------------ | ------------------ | -----------: | -------------: |
| events           | 1.0〜1.5 ターン/施設（正常完走した3回）                | 20〜25施設         |       30〜38 |         22〜30 |
| movies ステップ2 | 2.78 ターン/行（2026-09-17 の16会場・60行・167ターン） | 前回 tier A/B 15件 |           42 |             18 |

**単価が1桁違うので、同じ上限を共有しない。** events は施設ページ1枚で複数行が確定するが、
movies の名画座は会場ページから特集ごとに深掘りが要る。

余裕を厚めに取るのは、通信障害の連鎖だけで1体が膨らむ実例があるためである
（2026-09-04 の lives は DNS不通・SSL証明書不一致・タイムアウトの連鎖で141ターン）。

#### 割りすぎると、今度は固定費が効いてくる

体を増やせば1体あたりのターンは減るが、**体ごとに固定文脈**（system prompt・CLAUDE.md・
抜粋23k・worklist）を払い直す。同じ167ターンぶんの仕事で見ると、

| 体数 | 1体のターン | 合計の文脈再送 | うち固定費 |
| ---: | ----------: | -------------: | ---------: |
|    1 |         167 |          33.6M |        20% |
|    3 |          56 |          15.8M |        42% |
|    8 |          21 |          10.2M |        66% |
...
**20体まで割ると固定費が8割を超え、合計はもう下がらない。** 上の上限（events 20〜25施設・
movies 15件）は、その手前に収まる大きさとして置いている。

#### 事前定義に移すのは、抜粋の置き場所のためではない

エージェント定義の本文は子のシステムプロンプトになるので、`skill_brief.py` の抜粋
（23k）をそこへ移せば、同型の兄弟がプレフィクスを共有してキャッシュ書き込みが減る。
だが減るのはそこだけで、**毎ターンの再送は置き場所を変えても1トークンも減らない**
（第9.3.12節と同じ理屈。抜粋は子の文脈に載る以上、どこに置いても送り直される）。

実測の係留点（167ターン＝33.6M）から求めた文脈再送の増え方で見積もると、
...
一方、抜粋を定義本文へ移して減るキャッシュ書き込みは、12体起動しても
33.6M の 0.87% にすぎない。**効くのはターン数の上限であって、プロンプトの置き場所
ではない。** したがって抜粋は従来どおりファイルで渡し（正本は `SKILL.md` 1箇所のまま）、
定義本文には固定の注意書きだけを置く方針としている。

### 9.3.16 同じ催しの重複は、uid が一致しない形でだけ生き残る

`append_rows.py` は `uid`（タイトル・会場・日付の正規化キー）が一致する行を追記時に
弾く。**弾けないのは、タイトルの表記が違う重複である。**

2026-08-30 の週次実行は、1回の実行の中で同じ展覧会を2つの経路から書き込んだ。
「既存行の再確認」が前週の整形済みの行を書き戻し、同じ実行内の「エリア軸の探索」が
同じ展覧会を新規として追記している。タイトルに `東京都美術館開館100周年記念` という
プレフィックスが付くかどうかだけの違いで `uid` が一致せず、新規側は
`price` が `2600` という裸の数字・`pref` 空欄・URLが会場のトップページという、
第1段階の基準を満たさない行だった。**3週間以上、362行になるまで誰も気づかなかった。**

同じ実行の同じ会場で、別の展覧会にも**同一の `2600`** が付いている。正しい観覧料は
`一般2,300円` と `一般1,200円` で、`2600` はそのどちらでもない——会場トップに
出ていた数字（友の会年会費・共通券など）を、会場内の複数の展覧会に使い回したとみられる。

この事故は2つの側面を持つので、検査も2つ置く方針としている（`validate_data.py`）。

- **単位を伴わない `price`**（`^[\d,]+$`）を WARNING で挙げる。「価格は整形して書く」は
  散文にあったが機械チェックが無く、**書いた瞬間には誰も気づけなかった**。
  比較に使う整数は `price_official` という別の列にあるので、`price` が裸の数字である
  必要は一度も無い
- **同一CSV内で「同じ会場 × 会期が完全に一致 × タイトルが近い」組**を WARNING で挙げる。
  判定には `rowkey.similarity()` ではなく編集距離だけを使う——あちらは「共通する文字が
  どれだけ含まれているか」も見るので、`ワークショップ` のような短い題が、その文字を含む
  だけの無関係な行すべてに一致してしまう。週をまたぐ差分照合（消えた行 × 現れた行）なら
  候補が絞られていて実害が無いが、同一CSV内では全対が候補になる

しきい値と条件は、すべて実データに当てて決めている。

- **会期は「重なり」ではなく「完全一致」**。重なりまで広げると、同じ会場で同時期に走る
  別の企画（スタジオツアーのアフタヌーンティーとハイティーなど）を毎週挙げ続ける
- **編集距離 0.70 以上**。2026-08-30 の重複を含む390行に当てると22組を挙げ、その中に上の2組が入る
- **短いほうが長いほうの先頭か末尾に丸ごと入っている組も拾う**。編集距離は短い題で届かない
  ——`菊池桃子 ライブ` と `菊池桃子 ライブ「Moments」` は実在する同じ公演の2行だが 0.67 にしかならない。
  今回の事故の形（プレフィックスの有無だけが違う）も、この条件にそのまま当てはまる
- **ただし短いほうが長いほうの 0.40 以上の長さであること**。これが無いと `ワークショップ` が、
  その語で終わる無関係な題すべてに一致する。0.50 では拾える組が1つしか増えないが、
  0.40 まで下げると実際の重複が3組増える（`開園120周年記念 三溪園大茶会` / `三溪園大茶会` など）

この長さ比は**判定を変えない足切りとしても働く**。SequenceMatcher の比は
`2*一致文字数/(len(a)+len(b))` で一致文字数は短いほうの長さを超えられないので、
長さの比 r に対する上限は `2r/(1+r)`——r=0.40 なら 0.571 で、0.70 には構造的に届かない。
落とした組が本来しきい値を超えることはないので、総当たりの計算量だけが下がる
（実データの最大グループは4件・全体で49対、`validate_data.py` 全体で 0.06 秒）。

現在の362行＋lives・movies では8組で、うち5組が実際の重複である。

**ERROR にはしない。** 同じ会場・同じ会期の別企画（スタジオツアーのアフタヌーンティーと
ハイティーなど）は実在するので、機械が確定できるのは「見るべき組がここにある」までである。

### 9.3.17 「前回CSVの言い換え」は、取得回数と突き合わせてしか見抜けない

2026-09-06 の events 収集で、東京105施設を2波に分けて Haiku のサブエージェントへ渡した。
片方は「72施設全体を処理した」として92件を返したが、実体は前回CSVの該当行をほぼ改変
なしで再出力し、`price_checked` を全件実行日に一律で書き換えていただけだった
（`_carry:"*"` を100%の行に付与、個別の会場ページへの取得は行われた形跡がない）。
もう1波の86件も前回CSVの pref フィルタ再出力に留まり、**両方とも破棄している**。

そのときセッション全体の取得回数は44回だった。145施設を回るには構造的に足りない数字だが、
**親がそれに気づいたのは、自分で個別にCSVを突き合わせた後である。** 判定に必要な数字は
すべて機械の側にあったのに、誰も突き合わせていなかった。

`tools/carry_audit.py` を `append_rows.py` から呼び、次の4つが揃ったときだけ警告する
方針としている。

1. 波が20行以上（小さい波は割合が揺れる）
2. `_carry:"*"` が8割以上
3. `price_checked` が実行日の行が8割以上
4. **前回の追記以降の取得回数が、この波の会場数より少ない**

**1〜3は、正しい再確認でもそのまま成立する。** 前回から続く催しを会場ページで確認し、
変わっていないことを確かめて持ち越すのは、このタスクの主要動線そのものである。
区別できるのは4だけで、**会場ページを開かずに「今日確認した」と書ける行は無い**。

取得回数は累計ではなく**前回の追記からの差分**で見る。累計は波を重ねるほど増えるので、
後の波ほど「足りている」と誤判定する側に倒れてしまう。差分を取るための基準値は
`budget.py` の状態ファイルに1キー足して持つ（`load()` は知らないキーをそのまま保持する）。

**止めない。警告だけを出して追記は通す。** 捏造かどうかを最終的に決めるのは中身であって
この4条件ではなく、正しい仕事を機械が止めてしまうほうが被害が大きいためである。
同じ理由で、計測そのものが立ち上がっていない回（カウンタが1つも動いていない）は
判定しない——「計測できない」を「取得0回」と読むと、その回の全ての波が疑われる。
...
assets/
  ...
  js/                         ES モジュール。ビルド工程なしでそのまま配信する
    ...
    ui-caladd.js              カレンダーに追加する前の日時選択シート（第5.10節）
...
.claude/
  ...
  agents/
    kanto-collector-worker.md 収集の波を担当する調査専用サブエージェントの定義（maxTurns: 60。第9.3.15節）
  ...
  hooks/                      規則を決定論的に守らせるフック（第9.1.5節）
    ...
    fetch-budget-guard.sh     波の途中でも、撤退の線を越えた取得を止める（PreToolUse:Bash|WebSearch。第9.3.9節）
...
tools/
  ...
  append_lineup.py            フェスの日割りラインナップの書き出し（`--rows` で lives.csv の行と1回の呼び出しにまとめられる。第12.12節）
  fill_apple_music.py         ラインナップの Apple Music リンクを一括で埋める
  budget.py                   その回の消費（検索・取得・待機・追記・経過時間）の実測（第9.3.1節）
  run_gate.py                 その回が「収集した回」の形をしているかの判定（第9.3.11節）
  carry_audit.py              波が「前回CSVの言い換え」になっていないかの判定（第9.3.17節）
...
docs/
  ...
  figures/weekly-routine-flow.svg  起動から公開までの全体の流れの図（第13.8節）
...
上の表の2行は、どちらも**2つの独立したコマンド呼び出しの間で実行が打ち切られる**と
必ず踏む。`lives.csv` 側の行（`lineup_id` を含む）は `append_rows.py`、`lineups.csv`
側の実体は `append_lineup.py` と、書き手が別れているためである。2026-09-04 の
lives 収集は、フェスの行を8件書いた直後にアカウントの利用上限で打ち切られ、
`append_lineup.py` を一度も実行しないまま終わった——`lineup_id` を持つ行だけが
残り、ラインナップは0件のまま検証に落ちて、その週の収集全体が巻き戻った。

そこで `append_lineup.py` に `--rows <path>` を足し、`lives.csv` 側の行の検証
（`append_rows.py` と同じ経路。二重管理しない）とラインナップ側の検証を**両方
通してから、両方を書く**ようにした。検証と書き込みの間に呼び出しが挟まらないので、
打ち切りが割り込める窓は実質的に無くなる（完全な原子性ではない——2つのファイルへの
書き込みそのものは別の `open()` なので、その間のプロセス強制終了は理論上あり得るが、
窓は「複数ターンにまたがる分」から「同一プロセス内の2回の書き込みの間」まで縮む）。

`known_lineup_ids()`（`lives.csv` が参照しているか）だけでなく、逆方向
（今回書く行の `lineup_id` が、渡されたラインナップにも既存の `lineups.csv` にも
無い＝書いた瞬間にダングリング参照になる）もこの呼び出しの中で検証する。
既に `lives.csv` にある行にラインナップを追記するだけの通常の使い方
（`--rows` を渡さない）は変えていない。
...
### 12.13 来日公演は、招聘元を独立した調査の層として持つ

このタブの調査は会場名簿（`venues.csv`）とプレイガイドの2層で組んでいるが、**海外アーティストの来日公演はその両方から落ちる。** 会場側のカレンダーは、興行主が押さえた段階では公演名を出さず「貸切」等の表記で止まることがあり、プレイガイドは受付が始まった公演しか載せない。結果として**「発表済み・受付前」の期間だけ、どの層にも引っかからない大型公演が生まれる。**

実際に、ベルーナドームの来日公演2公演が2026-08-12の回で収集されたあと、2026-08-29の回で処分の記録なく消えている。会場名簿側の `last_checked` は同じ日に更新されており、**会場は見に行ったうえで公演が拾えていない**。招聘元の公演一覧（この2公演の出典も招聘元だった）を毎回見る層があれば、少なくとも「消えたことに気づける」状態になる。

そのため `data/sources.json` の `live` に招聘元5社（CREATIVEMAN PRODUCTIONS・ウドー音楽事務所・LIVE NATION H.I.P.・キョードー東京・indienative）を置き、収集スキルのステップ3に**全て開くことを「今週かならず産む成果」の下限として**明記している。いずれもURLが分かっているので検索の枠を消費せず、robots.txt もすべて許可している。

> `sources.json` は「調べたサイト一覧」として画面にも出るため、**ここに足すことは利用者への表示と着手前の robots.txt 確認（`check_robots.py`）の両方を同時に満たす**。逆に、スキル本文にだけ書いて `sources.json` に足さないサイトは、robots.txt の事前確認を素通りする（個々のURLは `fetch_gate.py` が取得の直前に判定するので取得自体は止まるが、着手前に一望できない）。

---

## 13. 無人実行の基盤（GitHub Actions self-hosted runner）

1回の実行が「起動 → 収集 → 検証 → 公開」とどう流れるかは、第13.8節の図にまとめてある。個々の判断の理由は第13.1〜13.7節にある。
...
**レーンは「どの機械の上で動くか」で分けている。** 止まったときに見る場所が、Pi（`journalctl`・`.claude/logs/`）と GitHub（Actions の実行一覧・Issue）で分かれるためである。GitHub 側の箱のうち `hosted runner` の印が付いたものだけが GitHub のクラウド上のマシンで処理を行い、Pi が止まっていても動く。印の無い箱（3・10・12）は、実行の受付と記録だけを担う。`weekly-collect.yml` と `routine-repair.yml` は GitHub に定義があっても、ジョブの中身は Pi の上で動く（`runs-on: [self-hosted, player-one-pi]`）。

#### 13.8.1 正常な回の流れ（1〜11）

| 番号 | どこで           | 何が                           | 何をするか                                                                                                                                                                                                            |
| ---- | ---------------- | ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | Pi               | `player-one-dispatch.timer`    | 水木金 02:30:00 JST に発火する（時刻帯は `Asia/Tokyo` と明示）                                                                                                                                                        |
| 2    | Pi               | `dispatch-routine.sh`          | `weekly-collect.yml` の起動を API で依頼する。一時的な失敗だけを再試行し、再送の前に「実は届いていたか」を確かめる（第13.3節）                                                                                        |
| 3    | GitHub           | `weekly-collect.yml`           | 実行を受け付け、`concurrency: weekly-routine` で `routine-repair.yml` と直列にしたうえで、ジョブを Pi に割り当てる                                                                                                    |
| 5    | Pi               | actions-runner（systemd 常駐） | ジョブを受け取り、`clean: false` で checkout してから `claude-routine.sh` を起動する（上限5時間）                                                                                                                     |
| 6    | Pi               | `claude-routine.sh`（前処理）  | ロック・`git fetch` と fast-forward・push 権限の事前確認・認証期限の確認を済ませ、`weekly-routine` の `schedule` ブロックから曜日に応じたスキル・モデル・同時実行数を決める。親向けの抜粋を作り、予算の計測を数え直す |
| 7・8 | Pi               | 親・子の Claude エージェント   | 収集する（下の第13.8.2節）                                                                                                                                                                                            |
| 9    | Pi               | `claude-routine.sh`（検証）    | Claude が終了したあと、`carry-rest --apply`・`purge_ended`・`run_gate`・`validate_data`・`diff_data` を回す。すべて通った回だけ commit して push する                                                                 |
| 10   | GitHub           | main ブランチ                  | 「週次データ更新」のコミットが入る                                                                                                                                                                                    |
| 11   | GitHub（hosted） | `pages.yml`                    | `weekly-collect.yml` の成功を `workflow_run` で受けて、GitHub Pages へデプロイする。10 の push は `GITHUB_TOKEN` によるもので `push` イベントを発火しないため、`workflow_run` で受けている（第13.2節）                |

**git 操作と検証を Claude ではなくシェル（6・9）に置いている。** Claude の実行が途中で終わっても pull と検証が必ず走り、検証に落ちたデータが push される余地を指示文ではなく仕組みで塞ぐためである（`claude-routine.sh` の冒頭のコメント）。Claude のセッションからは git の書き込みがフックで拒否される。
...
|                | 親エージェント（7）                                                                                                | 子エージェント（8）                                                                  |
| -------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------ |
| 起動のされ方   | `claude-routine.sh` が `claude -p "/weekly-routine"` で起動する                                                    | 親が `subagent_type: "kanto-collector-worker"` で前景に起動する。1つの波に同時2〜3体 |
| 役割           | 棚卸し・分割・起動・追記・検証。自分ではページを取得しない                                                         | 渡された範囲（名簿の一部・前回行）の調査だけ                                         |
| 読む手順書     | 親向けの抜粋 `temp/brief-parent-*.md`（行の書き方を除いたもの）                                                    | 子向けの抜粋 `temp/brief-*.md` と、担当範囲の前回行 `temp/worklist-*.md`             |
| 書いてよいもの | `append_rows.py` 経由の `data/*.csv`、名簿、処分の記録                                                             | `temp/rows-<波>.jsonl` だけ（1件ごとに追記する）。`data/` には書かない               |
| 返すもの       | 充足率・変化・打ち切った工程の報告                                                                                 | 書いたファイルのパスと件数だけ                                                       |
| 上限           | 文脈再送の線（25M で新しい波を止め、40M で撤退）                                                                   | 1体60ターン（`maxTurns`）。孫エージェントは起動できない                              |
| モデル         | Sonnet（`schedule` ブロックの3列目）                                                                               | `schedule` ブロックの5列目（2026-09-22 時点ではすべて Sonnet。第9.3.8節）            |
| 規則の縛り方   | 不変規則（`invariants.md`）をシステムプロンプトに載せ、フックで git・予算・取得・全文 Read を強制する（第9.1.5節） | 同じフックに加え、返答の形を `SubagentStop` フックが検査する                         |

親の流れは次のとおりである（手順の正本は `.claude/skills/weekly-routine/SKILL.md` と各収集スキルの「実行手順まとめ」）。

1. 親向けの抜粋を読む
2. `append_rows.py <ds> --init` で CSV を空にする。前回の CSV は `data/.prev/` へ退避され、「前回分」として以後の比較に使われる
3. `prev_rows.py <ds> --worklist` で前回行を棚卸しし、名簿と前回行を子に渡す範囲に割る
4. `skill_brief.py` で子への手順書を作り、波を投げる
5. 波が返ったら、子の JSONL を `append_rows.py` で CSV に追記する。必要なら次の波を投げる
6. 終了工程（`purge_ended`・`prev_rows --dispose`・`--carry-rest`・`diff_data`・`validate_data`・`report_stats`）を、調査をどこで打ち切っても必ず全部通す
...
### 13.9 失敗した回の自動調査（routine-investigate.yml）

`routine-repair.yml` は機械的な後始末しかできず（第13.4節）、判断の要る原因追及は人に返していた。深夜に無人で落ちた回の原因が、人が気づくまで手つかずで残るのは割に合わないため、**失敗した回に Claude を起動して原因を調べさせる経路**を別に持つ。`routine-repair.yml` とは排他ではなく、どちらも `concurrency: weekly-routine` に入って直列に走る。

#### 13.9.1 Issue の作成をトリガーにしない
...
収集が落ちる原因の上位は Claude 自身（認証の失効・アカウントの利用上限）で、その回は調査役の Claude も同じ理由で起動できない。**修復経路が「壊れているもの」に依存してはいけない**ため、`investigate-routine.sh` は最初に `claude-routine.sh --check-env` を回す。これは claude が「在るか」ではなく「動くか」を見るので、ここで切り分けられる。落ちた場合は調査を諦め、`--check-env` の出力を載せた Issue だけを残して正常終了する。

#### 13.9.3 push はスクリプトが行い、Claude には触らせない

このリポジトリは「Claude が書き、スクリプトが検証して push する」という分業を既に持っている（`invariants.md` と `block-git.sh`）。調査ルーチンは **main へ直接 push する**ため、push してよいかの判断を、判断を誤りうる側に置かない。差分は次の5つの関門を全部通ったときだけ push される。

| 関門     | 何を見るか                                                                                          |
| -------- | --------------------------------------------------------------------------------------------------- |
| 禁止パス | `.claude/hooks/` `.claude/routines/` `.github/workflows/` `tools/run_all_tests.py` に触れていないか |
| `data/`  | 収集の領分を書き換えていないか（持ち越し判定が前提にする「前回の行」がずれるため）                  |
| 規模     | 3ファイル・80行を超えていないか                                                                     |
| 弱体化   | テストの削除、`continue-on-error` / `\|\| true` / `--no-verify` の追加が混じっていないか            |
| ゲート   | `tools/run_all_tests.py` が全部通るか                                                               |

**既定は「診断のみ」で、push はこの5つを自力で通過したときだけ起きる例外である。** この非対称が、人のレビューを挟まずに main へ入れてよいと言える根拠になっている。1つでも引っかかれば診断だけが Issue に残り、実行そのものは正常終了する。

禁止パスの1つ目は**自分の拘束具**である。フック・規則・ワークフロー・ゲート本体を書き換えられる位置にある以上、そこへ手を入れることは規則違反ではなく判断の放棄として扱い、機械的に拒否する。

起動前の作業ツリーの汚れ（失敗した収集回が残した書きかけの `data/*.csv`）を基準線として控え、そこに載っていたパスは調査の成果に数えない。**収集の書きかけを巻き込んで push すると、検証を通っていないデータがそのまま公開されるため**である。

#### 13.9.4 調査は収集を起動しない

`investigate-routine.sh` は `weekly-collect.yml` を起動しない。起動すると「調査 → push → 再収集 → 失敗 → 調査」が閉じた輪になるためである。再実行を水木金の定期回に任せることで、アカウントの利用上限の残量に頼らず、**設計だけで輪を開いたままにしている**（利用上限は既に収集を殺した実績のある共有資源で、それを遮断器に使うと、枯渇して止まるのは調査ではなく収集の取り分になる）。

#### 13.9.5 収集向けの門を調査で発火させない

`read_gate.py` / `reply_gate.py` / `bash_gate.py` / `fetch_mix.py` は収集向けに調整されており、とくに `read_gate.py` は大物の全文 `Read` を拒否する。調査ではソースやログを読む必要があるため、これらを発火させると仕事そのものが妨げられる。そこで調査ルーチンは `CLAUDE_ROUTINE` ではなく **`CLAUDE_INVESTIGATE` を立てる**。**git の禁止（`block-git.sh`）だけは両方に効かせる**——どちらのルーチンも検証を飛ばした push が困る度合いは変わらず、main へ直接 push する調査の方がむしろ重いためである。

#### 13.9.6 規則と手順を分ける

第13.8節の収集と同じく、**不変規則は `.claude/routines/investigate-invariants.md`（`--append-system-prompt-file` でシステムプロンプトに載る）**、**手順は `.claude/skills/routine-investigate/SKILL.md`（`-p` で会話に展開する）** に分ける。分岐の基準は話題ではなく「圧縮で消えてよいか」である。検査を弱めない・安全装置を書き換えない・規模の上限といった規則は、消えた瞬間に不正な push が通るため最後のターンまで残す必要がある。一方、失敗回の特定方法や Issue の書式は、消えても手順を思い出せないだけで事故にはならない。
