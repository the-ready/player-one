...
**コミットの門（`claude-routine.sh`）には置かない。** あちらで落とすと `data/` が巻き戻り、価格が薄いことを理由に**その週の収集がまるごと消える**——薄い週より悪い結果になる。Stop フックは2周目に素通しするので、これは「捨てずにその場で直させる、1回だけの強い催促」である（第9.1.5節）。
...
したがって、**親は Sonnet・子は Haiku**（events）としている。曜日→スキル→モデルの対応表（`.claude/skills/weekly-routine/SKILL.md` の `schedule` ブロック）に子のモデルの列を持ち、`claude-routine.sh` が `CLAUDE_CODE_SUBAGENT_MODEL` として渡す。**この環境変数は Agent ツールの `model` 引数より優先される**ので、親が何を指定しても子は表のモデルになる——親の判断に委ねない。
...
`weekly-routine` の対応表が決めており（第9.3.8節）、`haiku` を指定する行がある。
