...
| `desc`・価格・クーポンがすべて空    | 「くわしく」ボタンごと出さない（中身のない箱を開かせない）                                                 |
...
| 640px以下 | 虫めがねアイコン（32px）に変化 | 縦幅が貴重。タップで右端から展開する         |
...
タブの区別は**タブ自体のUIと、カードの中身（左上のカテゴリバッジの色・タグ・CTAの文言）**が担う。カテゴリタグの文字色をすべて白に統一しているのは可読性と一貫性のためである（第8.3節）。
...
WCAG準拠のため、カテゴリ色の一部を暗く／明るく調整した。**カテゴリタグの文字色はすべて白で統一している**——「体験・アクティビティ」だけ濃い文字色、といった不統一は、利用者が意味の違いを読み取ろうとして混乱するため。統一を優先し、背景色側を白文字が乗る明度まで調整した。
...
| 親はすべての結果を書き終えるまでターンを終えない          | 追記していない調査結果は、最初から無かったのと同じ（`COLLECTION-PROTOCOL.md` 第10.2節）                                                      |
...
ときの倒し方は既にすべての門で決まっていて（`--gate` は 2 を返して通す、`--report` は
...
変わるので、2か所に書くと片方だけ更新されて食い違う。**トークンのために調査品質を
...
側の実体は `append_lineup.py` と、書き手が分かれているためである。2026-09-04 の
