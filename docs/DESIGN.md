...
そのうえで、**ポスターがあるはずの位置には出演者名を組んだタイポグラフィ・ブロックを置く**（第13.7節）。文字は事実情報なので権利上の制約がなく、フェスにおいては小さく表示されたポスター画像よりも情報量が多い（画像からは出演者を読み取れない）。
...
| `closed` | 完売・終了・締切超過 | 「受付終了」（グレー。取れるように見せない） |
...
### 13.5 ポスターの位置に出演者名を組む

画像を扱わない判断は第7.1節に書いた。ここでは**代わりに何を置いたか**を記す。

カード上部、映画ならポスターがある位置に、**出演者名を主ジャンルの色で組んだ「ラインナップ・ブロック」**を置いている。

- 出演者が1組なら `ARTIST`、複数なら `LINE UP` のラベルを付ける
- 表示は4組まで。超える分は「ほか◯組」に畳む
- 3組以上のときは1組あたりの文字を落として、行数で見せる（`.lineup.many`）
- `artists` が空の行では公演名を組む（枠だけが空で残らないようにする）

**これは「載せられないから代わりに」ではなく、ライブの一覧としてはこちらのほうが正しい。** フェスのカードにポスター画像を出しても、小さくてラインナップは読めない。文字なら読める。
