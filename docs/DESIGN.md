...
| 固定物                        | 以前                           | いま                         |
| ----------------------------- | ------------------------------ | ---------------------------- |
| 親が読む手順書                | SKILL.md 全文（60〜85千字）    | 親向けの抜粋（30〜50千字）   |
| 親が持つ前回分の棚卸し        | 全行（8〜29千字）              | `--summary`（0.7〜1.1千字）  |
| 子が持つ担当分の棚卸し        | 親が指示に**貼る**             | `temp/worklist-*.md` を Read |
| 子に積み上がる取得結果（1件） | 上限20,000字（未到達＝無制限） | 上限4,000字                  |
...
**46〜58%減る**（events で104,322字→43,834字）。
...
**取得結果の上限は、実測の分布を見て決めている。** 4,000字で切ると、実測68件の
うち切られるのは10件（全体の9%）で中央値のページには触れない。2,000字なら35%
減るが、そこは中央値（2,121字）を切りにいく線で、一覧の後半に載っている公演や
料金の行を落とす——**トークンのために調査品質を落とす**のは、この一連の仕組みで
最もやってはいけない誤りである（第9.1.5節の「出ているだけでは守られない」と
同じく、削るときも壊れ方の向きを選ぶ）。
...
全面英語化は後者の形になるので採らない。実測では `kanto-event-collector` の
本文74,530字のうち42,101字が既にASCII（ファイル名・コマンド・列名）である。

**トークンの差は根拠にならない。** 日英のトークン数の差は約1.2倍まで縮んでおり、
英訳で浮くのは総量の2割弱にとどまる。第9.3.12節の抜粋（読み手ごとに分ける）が
40%削るのに対して桁が1つ小さく、削減を理由に言語を選ぶ段階ではない。
...
`kanto-live-collector` は1,067字で上限を超えていたため、会場種別の列挙
（`domes, arenas, halls, livehouses, outdoor festival sites`）を落として1,008字と
している——会場の種別は `data/venues.csv` の `kind` 列（`dome` / `arena` /
`hall` / `livehouse` / `outdoor`）が持っており、スキルを選ぶ手がかりでは
ないためである。
