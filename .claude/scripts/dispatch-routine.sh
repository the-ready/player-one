...
#
# この回が起動できなかったときは、hosted runner 上の collect-fallback.yml が遅れて代わりに
# 起動し、Issue で知らせる。ここで粘りすぎると予備と二重に起動しうるため、再試行は
# 予備の発火（02:50 JST）より十分前に打ち切る。
#
# ============================================================
...
for delay in 0 $RETRY_DELAYS; do
  ...
  case "$code" in
    ...
    *)
      ...
      # 待っても直らない。粘らずに落とし、予備起動と Issue に委ねる。
...
log "ERROR: $attempt 回試みましたが起動できませんでした。collect-fallback.yml の予備起動に委ねます"
exit 3
