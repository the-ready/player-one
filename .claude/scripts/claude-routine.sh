...
# 起動元のセッションIDを引き継がない。
#
# `CLAUDE_CODE_SESSION_ID` は Claude Code が自分の記録の場所を示すために export
# する。**対話セッションの中からこのスクリプトを起動すると、子の `claude` が
# それをそのまま受け継ぐ**ので、新しいセッションが起動元の記録を自分のものだと
# 名乗ることになる。`budget.py` はIDで記録を引くので、以後すべての計測——
# 文脈再送・親子の内訳・最長の子のターン数——が**起動元の数字**になる。
#
# 結果は 2026-09-04 20:50 と同じである。あのときは `transcript_files()` の
# 「いちばん新しい記録」へのフォールバックが対話セッションの 44.8M を拾い、
# 「撤退の手順に入ってください」を文脈の先頭に載せて、検索0回・取得0回の回を
# 生んだ。フォールバックは廃止したが、**環境変数という別の経路が残っていた**
# ——実測では、対話セッションから起動した回が起動元の 38.4M を自分の消費として
# 読み、`agent-guard.sh` が最初の波を拒否している。
#
# cron から起動する限りこの変数は無いので、ふだんは効かない。効かないものを
# 外しておくのは、**効いたときに何が起きるかが「計測が全部嘘になる」だから**である。
...
#
# 【モデル】親は3スキルとも Sonnet。子は表の5列目が決める（2026-09-22時点、
# events も含め全スキルSonnet——eventsの子は本来Haikuで足りる判断だが、費用実測の
# ため一時的にSonnetにしている。戻す判断が要れば表の該当行だけ書き換えればよい。
# 詳細は docs/DESIGN.md 第9.3.8節）。
#
