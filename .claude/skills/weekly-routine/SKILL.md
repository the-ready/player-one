...
- **波は `subagent_type: "kanto-collector-worker"` で起動する。** 事前定義したエージェント
  （`.claude/agents/kanto-collector-worker.md`）で、**`maxTurns: 60` が入っている**。
  `general-purpose` で投げると——`subagent_type` を書かないと既定でそうなる——この上限は効かない。
  **指定の無い波は `agent-guard.sh` が拒否する。**
  モデルは定義に書いていないので、これまでどおり `CLAUDE_CODE_SUBAGENT_MODEL` が決める
- **1体が回るターン数は60でハーネスが打ち切る。** 1つの文脈でN回呼ぶと入力はNの2乗で増えるので、
  担当範囲を小さく割るだけでトークンが減る（実測の係留点 167ターン＝33.6M を3体に割ると 15.8M）。
  **波を受け取ると `PostToolUse` フックが `budget.py --report`（「最長の子◯ターン」を含む）を
  その場で差し込む**ので、次の波の分け方はその数字で決める。
  **打ち切りは範囲の途中でも起きる**が、子には「1件書けるたびに `temp/rows-*.jsonl` へ追記する」と
  指示してあるので、そこまでの成果は残る。残りは次の波か `--carry-rest` で片付ける
- **1体に渡す範囲の大きさは、スキルの抜粋に具体的な上限がある**（events は1波あたり20〜25施設、
  movies は前回の tier A/B が合計25件まで）。`maxTurns` は上から止めるだけで、
  **入る大きさに割る仕事は親が先にやる**
